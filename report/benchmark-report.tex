...
Over eighteen rounds of optimisation---progressing from algorithmic
...
SIMD primops, to register-blocked micro-kernels and panel-based blocked
factorisations---\texttt{linear-massiv} now \textbf{outperforms hmatrix
(OpenBLAS/LAPACK) in all nine benchmarked operation categories} at
$500{\times}500$: GEMM is $10\times$ faster single-threaded
...
($0.021\times$); eigenvalue decomposition is $2\times$ faster
($0.47\times$); and SVD is $1.3\times$ faster ($0.76\times$).
...
Table~\ref{tab:final-results} presents the final Round~18 benchmark
results across all thirteen benchmarked operation/size combinations.

\begin{table}[h]
\centering
\caption{Final performance: \texttt{linear-massiv} vs.\ \texttt{hmatrix}
(OpenBLAS/LAPACK), Round~18.  Ratios $< 1$ indicate \texttt{linear-massiv}
is faster.}
...
eigenSH $100{\times}100$    & 2.24\,ms  & 2.64\,ms       & $1.18\times$ & --- \\
eigenSH $200{\times}200$    & 17.4\,ms  & 20.7\,ms       & $1.19\times$ & --- \\
eigenSH $500{\times}500$    & 257\,ms   & 121\,ms        & $0.47\times$ & $2.1\times$ \\
SVD $100{\times}100$        & 3.62\,ms  & 6.43\,ms       & $1.78\times$ & --- \\
SVD $200{\times}200$        & 23.9\,ms  & 36.7\,ms       & $1.53\times$ & --- \\
SVD $500{\times}500$        & 351\,ms   & 267\,ms        & $0.76\times$ & $1.3\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
operation categories}.  Two categories (eigenSH and SVD) still trail at
$100\text{--}200$, where Level-2 tridiagonalisation and $A^\top A$
formation overhead dominate.
...
\toprule
\textbf{Operation} & \textbf{R1} & \textbf{R3} & \textbf{R6} & \textbf{R10} & \textbf{R17} & \textbf{R18} \\
\midrule
GEMM $100{\times}100$    & $329\times$     & $0.50\times$ & $0.50\times$ & ---          & $0.16\times$ & $0.15\times$ \\
LU solve $100{\times}100$ & $295\times$    & ---          & $0.37\times$ & $0.37\times$ & $0.68\times$ & $0.68\times$ \\
Cholesky $100{\times}100$ & $240\times$    & ---          & $0.34\times$ & $0.34\times$ & $0.63\times$ & $0.48\times$ \\
QR $100{\times}100$       & ${\approx}260\times$ & ---    & $0.13\times$ & ---          & $0.022\times$& $0.021\times$ \\
eigenSH $100{\times}100$  & $897\times$    & ---          & $0.93\times$ & $0.97\times$ & $1.04\times$ & $1.18\times$ \\
SVD $100{\times}100$      & $887\times$    & ---          & $3.28\times$ & $2.38\times$ & $1.80\times$ & $1.78\times$ \\
eigenSH $500{\times}500$  & ---            & ---          & ---          & ---          & $0.51\times$ & $0.47\times$ \\
SVD $500{\times}500$      & ---            & ---          & ---          & ---          & $0.90\times$ & $0.76\times$ \\
\bottomrule
\end{tabular}
\end{table}

The trajectory reveals three distinct phases:
\begin{enumerate}
...
The eighteen rounds of optimisation yield several cross-cutting
...
The eighteen rounds of optimisation have substantially changed the
...
\item[\texttt{hmatrix}] Still relevant for eigenSH and SVD at
  $n = 100\text{--}200$, where it retains a $1.2\text{--}1.8\times$
  advantage.  Also appropriate when existing codebases depend on its API,
  or when the full breadth of LAPACK routines (sparse solvers, banded
  systems, generalised eigenproblems) is needed---operations that
  \texttt{linear-massiv} does not yet implement.

\item[\texttt{linear-massiv}] Now the performance leader in all nine
  benchmarked categories at $500{\times}500$, and in seven of nine at all
  sizes.  Offers compile-time dimensional safety via \texttt{KnownNat},
...
After eighteen rounds of optimisation, \texttt{linear-massiv}
outperforms hmatrix (OpenBLAS/LAPACK) in seven of nine categories
at all benchmarked sizes.  The two remaining gaps---eigenSH at
$n = 100\text{--}200$ ($1.18\text{--}1.19\times$) and SVD at
$n = 100\text{--}200$ ($1.53\text{--}1.78\times$)---and the
potential to deepen existing advantages define the roadmap below.

Round~18 empirically evaluated and eliminated three previously
proposed targets: D\&C eigensolver crossover (numerical failures at
$n \geq 128$), GEMM $k$-loop unrolling (register spills, 20--30\%
regression), and svdGKP as a drop-in replacement ($22\text{--}24\times$
slower than svdAtAP).  The revised analysis below accounts for these
findings and proposes nine concrete targets in three priority tiers.

%% ------------------------------------------------------------------
\subsection{1.\ Multi-Shift QR with Aggressive Early Deflation for eigenSH}
\label{sec:future-multishift-qr}

\paragraph{Current state.}
The tridiagonal QR iteration (\texttt{rawTridiagQRLoopCM},
Symmetric.hs) uses single-shift implicit Wilkinson shifts with one
Givens bulge chase per step.  LAPACK's \texttt{DSTEQR} uses the same
algorithm, but its Fortran implementation has tighter constant
factors.  At $n = 100\text{--}200$, QR iteration accounts for
${\sim}40\%$ of \texttt{eigenSH} time (the remainder being
tridiagonalisation and Q~accumulation), and the current implementation
requires ${\sim}5$ iterations per eigenvalue on average.

\paragraph{Proposed optimisation.}
Implement multi-shift QR (2 or 4 shifts per step) following
Francis's implicit double-shift algorithm~\cite{gvl4}, combined with
\emph{aggressive early deflation} (AED)~\cite{gvl4}: after every few
QR steps, scan the subdiagonal for entries within a factor of
$\epsilon \|T\|_F$ of zero and deflate them via targeted Givens
rotations before they converge to machine zero.  This reduces the
average iterations per eigenvalue from ${\sim}5$ to ${\sim}2\text{--}3$.

\paragraph{Expected impact.}
$1.3\text{--}1.5\times$ QR iteration speedup, translating to
$1.12\text{--}1.20\times$ overall eigenSH improvement at
$n = 100\text{--}200$.  Combined with other eigenSH optimisations,
this targets bringing ratios below $1.0\times$.

%% ------------------------------------------------------------------
\subsection{2.\ Blocked Bidiagonalisation for SVD}
\label{sec:future-blocked-bidiag}

\paragraph{Current state.}
\texttt{svdAtAP} (SVD.hs) forms $A^\top A$ via GEMM, then
eigendecomposes the $n{\times}n$ product.  This squares the condition
number ($\kappa(A^\top A) = \kappa(A)^2$) and requires two
$O(n^3)$ phases.  The alternative \texttt{svdGKP} (SVD.hs:322)
bidiagonalises~$A$ directly via Golub--Kahan, but Round~18 benchmarks
confirmed it is $22\text{--}24\times$ slower due to per-element
Householder accumulation (a Level-2 bottleneck).  SVD at
$100{\times}100$ is $1.78\times$---the widest remaining gap.

\paragraph{Proposed optimisation.}
Apply the panel-based blocked WY strategy from
\texttt{panelTridiagP} (Symmetric.hs) to \texttt{svdGKP}'s
bidiagonalisation:
\begin{itemize}
\item Pack $\mathtt{nb}$ left and right Householder vectors into
  panels, compute T-factors via GEMM, and apply block reflectors
  via Level-3 GEMM---converting the Level-2 bottleneck to Level-3.
\item For the bidiagonal QR iteration phase, apply the SIMD Givens
  column-major transpose strategy to both $U$ and $V$, and
  implement aggressive early deflation on the bidiagonal.
\item Use a tighter convergence tolerance scaled by
  $\|B\|_F$ rather than per-element thresholds.
\end{itemize}

\paragraph{Expected impact.}
$1.5\text{--}2.5\times$ SVD improvement at medium sizes, potentially
bringing SVD $100{\times}100$ from $1.78\times$ to ${\sim}0.7\text{--}1.0\times$.

%% ------------------------------------------------------------------
\subsection{3.\ Scalar QR Path for Small eigenSH}
\label{sec:future-scalar-qr}

\paragraph{Current state.}
\texttt{symmetricEigenP} (Symmetric.hs:706--710) transposes $Q$ to
column-major, runs the SIMD Givens QR loop, then transposes back.
The two $O(n^2)$ transposes add ${\sim}5\text{--}8\%$ overhead for
$n = 100$, but the SIMD Givens benefit is marginal when QR
iteration is only ${\sim}40\%$ of total time.

\paragraph{Proposed optimisation.}
Add a size-based dispatch: for $n < 100$, skip the column-major
transpose and use a scalar Givens QR loop that operates directly on
the row-major $Q$ matrix.  The scalar loop avoids the transpose
overhead and SIMD setup costs, which dominate at small sizes.

\paragraph{Expected impact.}
$1.05\text{--}1.10\times$ eigenSH improvement at $n \leq 100$,
directly targeting the $1.18\times$ ratio.

%% ------------------------------------------------------------------
\subsection{4.\ Reduced Iteration Count for svdAtAP}
\label{sec:future-svd-maxiter}

\paragraph{Current state.}
\texttt{svdAtAP} (SVD.hs:148) calls \texttt{symmetricEigenP} with
$\mathtt{maxIter} = 10n$ iterations.  For $n = 100$, this budgets
1000~QR steps, but typical convergence requires ${\sim}5$ iterations
per eigenvalue (${\sim}500$ total).  The excess budget wastes time
on per-step convergence checks after all eigenvalues have converged.

\paragraph{Proposed optimisation.}
Reduce to $\mathtt{maxIter} = \max(20, 5n)$ with an early-exit check
that terminates when the largest remaining subdiagonal entry is below
the convergence tolerance.  For $n = 100$ this reduces the budget
from 1000 to 500 while maintaining identical accuracy.  Additionally,
scale the tolerance adaptively: $\mathtt{tol} = \max(10^{-14},
10^{-12} \cdot \|A^\top A\|_F)$ to improve convergence for
well-conditioned matrices.

\paragraph{Expected impact.}
$1.05\text{--}1.15\times$ SVD improvement from faster eigenSH
convergence, cascading through the $A^\top A$ pipeline.

%% ------------------------------------------------------------------
\subsection{5.\ D\&C Eigensolver Numerical Stabilisation}
\label{sec:future-dc-stable}

\paragraph{Current state.}
The divide-and-conquer eigensolver (\texttt{symmetricEigenPDC},
Symmetric.hs:724) is implemented and tested at small sizes ($n \leq 50$)
but produces reconstruction errors of $166\text{--}573$ at $n \geq 128$
(Round~18).  The crossover remains at $n = 100{,}000$, effectively
disabled.  D\&C's $O(n^2)$ average-case complexity (vs.\ QR's $O(n^3)$)
makes it critical for large-matrix competitiveness.

\paragraph{Proposed optimisation.}
Rewrite the secular equation solver using the Gu--Eisenstat
algorithm~\cite{gvl4}, which guarantees orthogonality of computed
eigenvectors via careful root-finding with Gragg's fixed-weight
formula and targeted reorthogonalisation.  Add mixed-precision
accumulation (compensated summation) in the rank-1 merge.
Re-enable D\&C at $n \geq 256$ after validation at $128$, $256$,
$512$, and $1024$.

\paragraph{Expected impact.}
$1.3\text{--}1.8\times$ eigenSH improvement at $n \geq 256$,
deepening the existing $0.47\times$ advantage at $500{\times}500$.
Asymptotically dominant for $n \geq 1000$.

%% ------------------------------------------------------------------
\subsection{6.\ Panel-Based LU and Cholesky Factorisation}
\label{sec:future-panel-lu-chol}

\paragraph{Current state.}
Both LU (LU.hs) and Cholesky (Cholesky.hs) use column-by-column
factorisation---Level-2 BLAS operations.  Current ratios are
$0.68\times$ (LU) and $0.48\times$ (Cholesky) at $100{\times}100$,
already faster than LAPACK.

\paragraph{Proposed optimisation.}
Implement LAPACK-style panel factorisation (DGETRF/DPOTRF):
factor a panel of $\mathtt{nb}$ columns via Level-2, then update the
trailing submatrix via TRSM and rank-$\mathtt{nb}$ GEMM (Level-3).
The $10\times$-faster GEMM micro-kernel amplifies the Level-2 to
Level-3 promotion.

\paragraph{Expected impact.}
$1.3\text{--}1.5\times$ improvement at $n \geq 100$, deepening
advantages to ${\sim}0.3\text{--}0.5\times$.

%% ------------------------------------------------------------------
\subsection{7.\ Adaptive Tridiagonalisation Panel Size}
\label{sec:future-adaptive-panel}

\paragraph{Current state.}
The tridiagonalisation panel size is fixed at $\mathtt{nb} = 48$
(Symmetric.hs:175) with a crossover at $n = 256$ from per-column
(Level-2) to panel (Level-3) mode.  For $n = 100\text{--}200$, the
per-column path is used, but its Q~accumulation threshold of $n = 256$
(Symmetric.hs:213) forces the simple per-row path for all benchmarked
sizes.

\paragraph{Proposed optimisation.}
Lower the blocked WY Q-accumulation threshold from 256 to 200, and
for the per-column path at $n = 100\text{--}255$, tune workspace
allocation (currently 3 temporary vectors of $n$ doubles each).
Reduce allocation churn by hoisting vector allocation outside the
column loop.

\paragraph{Expected impact.}
$1.05\text{--}1.10\times$ eigenSH improvement at $n = 200$, where
the Q-accumulation overhead is a measurable fraction of
tridiagonalisation time.

%% ------------------------------------------------------------------
\subsection{8.\ 2D Parallel GEMM Tiling}
\label{sec:future-2d-gemm}

\paragraph{Current state.}
\texttt{matMulPPar} (Level3.hs) partitions by rows: each of $p$
threads computes $\lfloor m/p \rfloor$ rows of $C$, reading the
entire $B$ matrix.  On a multi-core system with $500{\times}500$
matrices, all threads contend for the same $2$\,MB $B$ in L3.

\paragraph{Proposed optimisation.}
Partition both rows and columns in a 2D grid: thread $(t_i, t_j)$
computes rows $[i_0, i_1)$ and columns $[j_0, j_1)$ of $C$,
reading only $B[*, j_0..j_1)$.  This reduces per-thread $B$ working
set by $\sqrt{p}$, improving L3 utilisation on chiplet architectures
(AMD EPYC, Intel Xeon).

\paragraph{Expected impact.}
$1.2\text{--}1.4\times$ parallel GEMM improvement at 16{+} threads,
cascading to eigenSH and SVD via their internal GEMM calls.

%% ------------------------------------------------------------------
\subsection{9.\ Cache Tile and Panel Size Auto-Tuning}
\label{sec:future-autotune}

\paragraph{Current state.}
GEMM tile size $\mathtt{bs} = 64$ (Kernel.hs) and tridiagonalisation
panel size $\mathtt{nb} = 48$ (Symmetric.hs) are compile-time
constants tuned on AMD Zen (32\,KB L1d, 512\,KB L2).  These are
...
a pre-validated lookup table.  Alternatively, implement a one-time
auto-tuning pass at library initialisation.
...
\midrule
\multicolumn{6}{l}{\textit{High priority --- closing remaining gaps}} \\
1 & Multi-shift QR / AED           & High   & High   & $1.12\text{--}1.20\times$ & eigenSH ($n = 100\text{--}200$) \\
2 & Blocked bidiagonalisation      & High   & High   & $1.5\text{--}2.5\times$   & SVD ($n = 100\text{--}200$) \\
3 & Scalar QR for small eigenSH    & High   & Low    & $1.05\text{--}1.10\times$ & eigenSH ($n \leq 100$) \\
4 & Reduced svdAtAP iterations     & High   & Low    & $1.05\text{--}1.15\times$ & SVD (all sizes) \\
\midrule
\multicolumn{6}{l}{\textit{Medium priority --- deepening advantages}} \\
5 & D\&C eigensolver stabilisation & Medium & High   & $1.3\text{--}1.8\times$   & eigenSH ($n \geq 256$) \\
6 & Panel LU/Cholesky              & Medium & Medium & $1.3\text{--}1.5\times$   & LU, Cholesky \\
7 & Adaptive tridiag panel size    & Medium & Low    & $1.05\text{--}1.10\times$ & eigenSH ($n = 200$) \\
\midrule
\multicolumn{6}{l}{\textit{Lower priority --- architectural improvements}} \\
8 & 2D parallel GEMM tiling        & Low    & High   & $1.2\text{--}1.4\times$   & Parallel GEMM \\
9 & Cache/panel auto-tuning        & Low    & Low    & $1.05\text{--}1.20\times$ & All (non-default HW) \\
\bottomrule
\end{tabular}
\end{center}

The four high-priority items target the two categories where
\texttt{linear-massiv} still trails LAPACK: eigenSH at
$n = 100\text{--}200$ ($1.18\text{--}1.19\times$) and SVD at
$n = 100\text{--}200$ ($1.53\text{--}1.78\times$).  Items~3 and~4
are quick wins (low difficulty), while items~1 and~2 require
substantial algorithmic work but address the root causes: too many QR
iterations (eigenSH) and the Level-2 bidiagonalisation bottleneck (SVD).

Three previously proposed targets were empirically eliminated in
Round~18 and are \emph{not} re-proposed: (a)~D\&C crossover tuning
(numerical instability at $n \geq 128$---item~5 proposes a
stabilisation rewrite instead); (b)~GEMM $k$-loop unrolling (GHC's
LLVM~17 backend spills registers when holding $> 14$ simultaneous
\texttt{DoubleX4\#} values); (c)~svdGKP as drop-in replacement
($22\text{--}24\times$ slower without blocked bidiagonalisation---
item~2 proposes the blocking prerequisite).

\paragraph{Not recommended at current benchmark sizes.}
Strassen/Winograd multiplication (breakeven at $n > 2000$; benchmarks
cap at $n = 500$), GPU acceleration (breaks the pure-Haskell premise),
and cache-oblivious algorithms (GHC recursion overhead negates the
theoretical benefit) are not expected to yield gains at the sizes
benchmarked here.
...
categories of numerical operations through eighteen rounds of optimisation.
