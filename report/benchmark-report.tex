...
After Round~21, \texttt{linear-massiv} achieves parity or better in
\textbf{all nine operation categories at $200{\times}200$ and above},
with the sole remaining deficit being SVD at $100{\times}100$ ($1.37\times$).
...
Table~\ref{tab:final-results} presents the final Round~21 benchmark
...
(OpenBLAS/LAPACK), Round~21.  Ratios $< 1$ indicate \texttt{linear-massiv}
is faster.}
...
SVD $100{\times}100$        & 3.23\,ms  & 4.41\,ms       & $\mathbf{1.37\times}$ & --- \\
SVD $200{\times}200$        & 25.4\,ms  & 22.7\,ms       & $\mathbf{0.89\times}$ & $1.1\times$ \\
SVD $500{\times}500$        & 346\,ms   & 212\,ms        & $\mathbf{0.61\times}$ & $1.6\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
operation categories}.  Round~21's D\&C eigensolver dispatch in
\texttt{svdAtAP} produced the largest SVD improvement since Round~6:
SVD $200{\times}200$ moved from a $1.17\times$ deficit to $0.89\times$
(now faster than LAPACK); SVD $500{\times}500$ improved from
$0.77\times$ to $0.61\times$ ($1.6\times$ faster than LAPACK).  The
sole remaining deficit is SVD at $100{\times}100$ ($1.37\times$, reduced
from $1.59\times$).
...
\toprule
\textbf{Operation} & \textbf{R1} & \textbf{R3} & \textbf{R6} & \textbf{R10} & \textbf{R17} & \textbf{R18} & \textbf{R19} & \textbf{R20} & \textbf{R21} \\
\midrule
...
SVD $100{\times}100$      & $887\times$    & ---          & $3.28\times$ & $2.38\times$ & $1.80\times$ & $1.78\times$ & $1.86\times$ & $1.59\times$ & $\mathbf{1.37\times}$ \\
SVD $200{\times}200$      & ---            & ---          & ---          & ---          & ---          & $1.53\times$ & $0.98\times$ & $1.17\times$ & $\mathbf{0.89\times}$ \\
eigenSH $500{\times}500$  & ---            & ---          & ---          & ---          & $0.51\times$ & $0.47\times$ & $0.48\times$ & $0.49\times$ & $=$ \\
SVD $500{\times}500$      & ---            & ---          & ---          & ---          & $0.90\times$ & $0.76\times$ & $0.82\times$ & $0.77\times$ & $\mathbf{0.61\times}$ \\
\bottomrule
\end{tabular}
\end{table}
...
The twenty-one rounds of optimisation yield several cross-cutting
...
\texttt{DBDSDC}).  The remaining $1.37\times$ SVD deficit at
...
The twenty-one rounds of optimisation have substantially changed the
...
\item[\texttt{hmatrix}] Still relevant for SVD at $n \leq 100$, where
  ...
  ($1.37\times$ at $100{\times}100$).  At $200{\times}200$ and above,
  \texttt{linear-massiv} is now faster.
...
After twenty-one rounds of optimisation, \texttt{linear-massiv}
outperforms hmatrix (OpenBLAS/LAPACK) in all nine categories at
$200{\times}200$ and above.  Rounds~20--21 implemented seven of the
nine optimisations proposed after Round~19, plus the D\&C bidiagonal SVD
and D\&C eigensolver dispatch that closed the SVD gap at $n \geq 200$.
The sole remaining deficit is SVD at $100{\times}100$ ($1.37\times$).

The roadmap below is revised to reflect the post-Round~21 state.
Items~1--7 from the previous roadmap are implemented
(Section~\ref{sec:round20}); the D\&C bidiagonal SVD and eigensolver
dispatch are implemented in Section~\ref{sec:round21}.  Remaining
opportunities focus on infrastructure scaling and future GHC
capabilities.
...
Round~20 (Sub-rounds 20a--20h), and Round~21 added D\&C bidiagonal SVD
and D\&C eigensolver dispatch.  The two remaining items (cache
...
\item \textbf{Raw \texttt{svdGKP} as drop-in default} ---
  \textit{partially addressed in Round~21.}  The D\&C bidiagonal SVD
  (Round~21a) improved \texttt{svdGKP} by ${\sim}20\%$, but it remains
  $2\text{--}3\times$ slower than \texttt{svdAtAP} due to higher
  constant factors in the bidiagonalisation + D\&C merge pipeline.
...
The remaining SVD deficit ($1.37\times$ at $100{\times}100$) reflects
...
categories of numerical operations through twenty-one rounds of optimisation.
...
($0.61\text{--}1.37\times$).  At $500{\times}500$---the largest
...
scaling to other hardware and future GHC capabilities.  Round~21's
D\&C eigensolver dispatch in \texttt{svdAtAP} proved the most impactful
single change for SVD since Round~6: the D\&C's GEMM-dominated
eigenvector accumulation leverages the $6\text{--}12\times$ faster GEMM
kernel, making it dramatically cheaper than QR iteration's scalar
Givens sweeps.
