...
\paragraph{Derived Work Attribution.}
The \texttt{linear-massiv} library was co-authored by Claude Opus (Anthropic).
The generated code, beyond the Claude Opus co-authorship attribution, should
be considered a derived work of the various algorithmic examples and reference
implementations drawn upon during development, including but not limited to:
LAPACK~\cite{lapack} (algorithm structures, testing methodology, numerical
stability techniques), OpenBLAS~\cite{openblas} (tiled GEMM micro-kernel
architecture, cache-blocking strategies, SIMD vectorisation patterns),
Golub \& Van~Loan~\cite{gvl4} (primary algorithmic reference), and
Higham~\cite{higham} (error analysis and stability frameworks).
...
then exceeding hmatrix/LAPACK performance in all nine categories.  The proposals
...
SVD gaps, while Rounds~14--18 pushed all categories past hmatrix/LAPACK through
...
\item \textbf{LU and Cholesky factorisation.}
  ...
  constant factor of hmatrix/LAPACK.
...
is itself $1.3\times$ slower than hmatrix/LAPACK at this size.  A Golub--Kahan
...
\item \textbf{Golub--Kahan bidiagonalisation SVD.}
  ...
  parity with hmatrix/LAPACK.

\item \textbf{Divide-and-conquer tridiagonal eigensolver.}
  Although the QR iteration now matches hmatrix/LAPACK at $50{\times}50$, at
...
\noindent
With BLAS and solver operations now faster than hmatrix/LAPACK and eigenvalue
...
than hmatrix/LAPACK (Table~\ref{tab:round7-svd}).  The Golub--Kahan
...
\item \textbf{Blocked WY Householder bidiagonalisation for SVD.}
  ...
  improvement, potentially bringing SVD within $1.5\times$ of hmatrix/LAPACK.
...
bringing the full eigenSH pipeline within $5\text{--}20\%$ of hmatrix/LAPACK.
...
\texttt{svdAtAP} now \emph{outperforms hmatrix/LAPACK} at large sizes.
...
than hmatrix/LAPACK.  The small-size ratios ($1.04\text{--}1.18\times$) are
...
SVD at $500{\times}500$ is now $\mathbf{10\%}$ faster than hmatrix/LAPACK.
...
\begin{tabular}{llr}
\toprule
\textbf{Category} & \textbf{Best ratio} & \textbf{Faster than hmatrix/LAPACK by} \\
\midrule
...
$2.1\times$ faster than hmatrix/LAPACK.  The improvement comes from both the
...
\item \textbf{Panel LU/Cholesky (Opt~6):} Already $1.8\text{--}2.1
  \times$ faster than hmatrix/LAPACK at $100{\times}100$.  Marginal improvement
  potential.  Deferred.
...
from $1.18\times$ (18\% slower than hmatrix/LAPACK) to $1.00\times$ (parity).
At $200{\times}200$, the improvement is even more dramatic:
$1.19\times \to 0.69\times$, a $1.7\times$ speedup making
\texttt{linear-massiv} $1.5\times$ faster than hmatrix/LAPACK.
...
%% ====================================================================
\section{Round~20: Fused DSYRK, Blocked SVD, D\&C Stabilisation, Panel Solvers, and GEMM Packing}
\label{sec:round20}

Round~20 implements seven of the nine optimisations from the
Chapter~\ref{sec:future-optimisations} roadmap in eight sub-rounds
(20a--20h), targeting SVD, eigenSH, LU, Cholesky, and GEMM performance
at all sizes.  Cache auto-tuning (Opt~8) is deferred as the development
machine's cache hierarchy is already the optimisation target; AVX-512
(Opt~9) is deferred pending practical validation
(Section~\ref{sec:future-avx512}).

\subsection{Implemented Optimisations}

\subsubsection{Sub-round 20a: Fused DSYRK Kernel (Opt~3)}

A new \texttt{rawSyrkLowerKernel} (Kernel.hs) computes $C = A^\top A$
directly, writing only the lower triangle and mirroring to the upper
in an $O(n^2)$ pass.  Uses the same tiled $4{\times}8$ SIMD FMA
micro-kernel structure as GEMM, with the key optimisation that only
tiles $(i, j)$ with $i \geq j$ are processed.  Eliminates the
transpose allocation of the previous \texttt{matMulAtAP} and halves
the flop count via symmetry exploitation.

\subsubsection{Sub-round 20b: Column-Major SIMD Givens for Bidiagonal QR (Opt~1 partial)}

The bidiagonal QR iteration in \texttt{svdGKP} (SVD.hs) now transposes
$U$ and $V$ to column-major layout before the QR iteration, applies
Givens rotations via the \texttt{DoubleX4\#} SIMD kernel
\texttt{rawMutApplyGivensColumnsCM}, and transposes back.  Aggressive
early deflation scans the bottom $w = \min(6, (h{-}l{+}1)/3)$
superdiagonal entries before each QR step, and stall detection bails
after 20~consecutive steps without deflation.  Size dispatch uses the
column-major path for $n \geq 10$.

\subsubsection{Sub-round 20c: Blocked Bidiagonalisation and Blocked WY Accumulation (Opt~1 main)}

The Golub--Kahan SVD path now uses blocked bidiagonalisation
(\texttt{bidiagonalizeBlockedP}, SVD.hs) with panel width
$\mathtt{nb} = \min(32, n/4)$ for $n \geq 32$.  Within each panel,
left and right Householder vectors are accumulated; after the panel,
trailing-matrix updates are applied via Level-3 GEMM using the compact
WY representation ($T$-factor + packed $Y$ matrix).  $U$ and $V$
eigenvector matrices are accumulated via blocked WY rather than
per-column scalar application, converting the $O(mn^2)$ Householder
accumulation from Level-2 to Level-3.

\subsubsection{Sub-round 20d: D\&C Eigensolver Stabilisation (Opt~2)}

The divide-and-conquer eigensolver (\texttt{symmetricEigenPDC},
Symmetric.hs) received three numerical robustness fixes:
\begin{enumerate}
\item \textbf{Perturbation-based deflation.}  When
  $\rho z_i^2 < \varepsilon \|A\|$, the eigenvalue perturbation is
  below machine precision.  The secular solver returns
  $\lambda_i = d_i$ exactly, causing the Gu--Eisenstat eigenvector
  formula to zero the dominant component.  A new deflation criterion
  deflates entries where
  $|z_i| < \sqrt{\varepsilon (1 + \|A\|) / \rho}$.
\item \textbf{Fixed-pole separation.}  The \texttt{fixedWeightLoop}
  now uses fixed pole positions \texttt{dLo}/\texttt{dHi} for
  close-pole contributions, independent of the narrowing bracket
  bounds.
\item \textbf{Close-$d$ Givens deflation.}  LAPACK dlaed2-style
  Givens rotations merge nearly-equal sorted $d$ values, preserving
  orthogonality.
\end{enumerate}
The D\&C crossover is set to $n = 1000$ because the merge-step GEMM
overhead does not amortise below this size.  The D\&C path is now
numerically stable (reconstruction error $< 5 \times 10^{-7}$ at
$n = 128$) but awaits further merge-step optimisation to be
competitive with QR at $n \leq 500$.

\subsubsection{Sub-round 20e: Panel LU and Cholesky Factorisation (Opt~5)}

Both LU and Cholesky now use panel (blocked) factorisation with
$\mathtt{nb} = 32$ for $n \geq 64$.

\textbf{Panel LU:} A new \texttt{rawLUEliminateColumnTo} kernel
restricts the trailing update to within the panel.  After each panel,
a triangular solve computes $U_{12}$ from the panel's unit lower
triangle, then a single GEMM updates the trailing submatrix:
$A_{22} \mathrel{-}= L_{21} U_{12}$.

\textbf{Panel Cholesky:} A new \texttt{rawCholColumnSIMDFrom} kernel
processes within-panel dependencies starting from the panel's first
column.  Before each panel, a GEMM applies contributions from all
previous panels: $G \mathrel{-}= L_{\text{prev}} L_{\text{panel}}^\top$.

\subsubsection{Sub-round 20f: Double-Shift QR (Opt~4)}

The symmetric tridiagonal QR iteration
(\texttt{rawTridiagQRLoop}/\texttt{rawTridiagQRLoopCM}, Symmetric.hs)
now supports an implicit double-shift bulge chase.  When the active
window $h - l \geq 4$, two consecutive Wilkinson shifts are computed
from the bottom $2{\times}2$ of the active block and applied in
succession.  The first sweep uses the original shift; if the bottom
element does not deflate, a second sweep with a recomputed shift is
applied immediately, halving the number of convergence-check overheads.

\paragraph{Result:} Negligible measurable impact.  The QR iteration
phase accounts for only $2\text{--}4\%$ of total eigenSH time
(tridiagonalisation dominates at $96\text{--}98\%$); even a $2\times$
speedup in QR iteration yields $< 4\%$ overall improvement, well
within benchmark noise.  The implementation is retained as it adds
no overhead to the non-double-shift path.

\subsubsection{Sub-round 20g: GEMM Micro-Panel Packing (Opt~6)}

For $\min(m, k, n) \geq 256$, the GEMM kernel now packs $B$ into
contiguous micro-panels before the micro-kernel loop.  The packed
variant (\texttt{rawGemmBISlicePackedBK}, Kernel.hs) uses a
$bk$-outer loop ordering: for each $k$-tile of width
$kc = \min(64, k{-}bk)$, the $B$ submatrix is packed into 8-column
panels in panel-major layout (stride~8 instead of stride~$n$), then
frozen via \texttt{unsafeFreezeByteArray\#} to enable pure
\texttt{indexDoubleArrayAsDoubleX4\#} reads.  This avoids State\#
threading through the $k$-loop, which was found to prevent GHC's
optimiser from fully unboxing the accumulator chain (a stateful
variant showed a $6.5\%$ regression).

\paragraph{Result:} GEMM $500{\times}500$ improved from
$0.092\times$ to $0.084\times$ ($8.7\%$ improvement).  Below the
crossover ($n < 256$), the unpacked path is used with no change.
The improvement cascades to all operations that call GEMM internally
(eigenSH, SVD, LU, Cholesky).

\subsubsection{Sub-round 20h: 2D Parallel GEMM Tiling (Opt~7)}

The parallel GEMM (\texttt{matMulPPar}, Level3.hs) now uses 2D grid
partitioning when $\mathtt{numThreads} \geq 4$ and both dimensions
$m, n \geq 128$.  A new column-restricted kernel
(\texttt{rawGemmBIBJSlice}, Kernel.hs) accepts $bjStart$/$bjEnd$
parameters, enabling each thread $(t_i, t_j)$ to compute only its
assigned row and column tile of $C$.  Grid dimensions are chosen
to minimise aspect ratio: $\lfloor\sqrt{p}\rfloor \times
\lceil p / \lfloor\sqrt{p}\rfloor \rceil$.  This reduces per-thread
$B$ cache working set by $\sqrt{p}$, improving L3 utilisation on
chiplet architectures.

\paragraph{Result:} No impact on single-threaded benchmarks (the
primary comparison target).  Parallel GEMM scaling is improved for
$\geq 4$ threads on matrices $\geq 128{\times}128$.

\subsection{Deferred Optimisations}

\textbf{Cache auto-tuning (Opt~8):} Detection of cache sizes at
runtime (\texttt{/sys/devices/system/cpu/} on Linux) to select
optimal tile/panel sizes.  Deferred because the development machine's
cache hierarchy is the only one benchmarked, so hard-coded sizes
($\mathtt{bs} = 64$) are already optimal for the measured results.
Would improve performance on different hardware (Apple M-series, Intel
Golden Cove).

\textbf{AVX-512 (Opt~9):} GHC has defined \texttt{DoubleX8\#} primops
since GHC~7.8, usable via the LLVM backend with \texttt{-mavx512f}.
Deferred due to practical obstacles: register pressure with 512-bit
types (untested with LLVM's allocator), limited AVX-512 hardware
availability, and thermal throttling on some Intel processors.  See
Section~\ref{sec:future-avx512}.
...
\begin{tabular}{lrrrc}
\toprule
\textbf{Operation} & \textbf{hmatrix} & \textbf{lm} & \textbf{Ratio} & \textbf{vs.\ R19} \\
\midrule
GEMM $100{\times}100$       & 1.06\,ms  & 183\,\textmu s & $0.17\times$ & $=$ \\
GEMM $200{\times}200$       & 10.9\,ms  & 1.30\,ms       & $0.12\times$ & $=$ \\
GEMM $500{\times}500$       & 220\,ms   & 18.5\,ms       & $\mathbf{0.084\times}$ & $\downarrow$ 0.092 \\
eigenSH $100{\times}100$    & 2.63\,ms  & 2.54\,ms       & $\mathbf{0.96\times}$ & $\downarrow$ 1.00 \\
eigenSH $200{\times}200$    & 16.1\,ms  & 10.1\,ms       & $\mathbf{0.63\times}$ & $\downarrow$ 0.69 \\
eigenSH $500{\times}500$    & 240\,ms   & 118\,ms        & $\mathbf{0.49\times}$ & $\downarrow$ 0.54 \\
SVD $100{\times}100$        & 3.74\,ms  & 5.96\,ms       & $\mathbf{1.59\times}$ & $\downarrow$ 1.86 \\
SVD $200{\times}200$        & 25.1\,ms  & 29.4\,ms       & $1.17\times$ & $\uparrow$ 0.98 \\
SVD $500{\times}500$        & 416\,ms   & 320\,ms        & $\mathbf{0.77\times}$ & $\downarrow$ 0.81 \\
LU solve $100{\times}100$   & 455\,\textmu s & 215\,\textmu s & $\mathbf{0.47\times}$ & $\downarrow$ 0.68 \\
Cholesky $100{\times}100$   & 307\,\textmu s & 158\,\textmu s & $0.51\times$ & $=$ \\
QR $100{\times}100$         & 140\,ms   & 2.75\,ms       & $0.020\times$ & $=$ \\
\bottomrule
\end{tabular}
\end{center}
...
The principal achievements of Round~20 are:

\textbf{GEMM packing (Sub-round 20g):} Micro-panel packing with
$bk$-outer loop ordering and \texttt{unsafeFreezeByteArray\#} for pure
$k$-loop reads reduces GEMM $500{\times}500$ from $0.092\times$ to
$0.084\times$ ($8.7\%$ improvement, now $11.9\times$ faster than
hmatrix/OpenBLAS).  The key insight is that State\# threading through
the packed $k$-loop prevents GHC from optimising the accumulator chain;
freezing each packed tile into an immutable \texttt{ByteArray\#} enables
\texttt{indexDoubleArrayAsDoubleX4\#} reads in a pure loop, recovering
the full optimisation benefit.  This improvement cascades: eigenSH
$500{\times}500$ improved from $0.54\times$ to $0.49\times$ and SVD
$500{\times}500$ from $0.81\times$ to $0.77\times$ via their internal
GEMM calls.

\textbf{LU solve (Sub-round 20e):} Panel factorisation reduces the
ratio at $100{\times}100$ from $0.68\times$ to $0.47\times$, a 31\%
improvement.  The Level-3 GEMM trailing update converts the
cache-unfriendly column-by-column elimination into a single
cache-friendly tiled operation.

\textbf{D\&C eigensolver stability (Sub-round 20d):} The
perturbation-based deflation criterion eliminates the catastrophic
orthogonality loss that disabled the D\&C path.  When
$\rho z_i^2 < \varepsilon |d_i|$, the secular solver returned
$\lambda_i = d_i$ exactly, causing the Gu--Eisenstat eigenvector
formula to produce $z_i^{(\text{new})} = 0$---zeroing the dominant
component.  The fix deflates these entries proactively, achieving
reconstruction errors $< 5 \times 10^{-7}$ at $n = 128$.  The D\&C
path is now stable but remains slower than QR at $n \leq 500$
($\texttt{dcCrossover} = 1000$) due to merge-step GEMM overhead.

\textbf{SVD $100{\times}100$ (Sub-rounds 20a--20c):} The fused DSYRK
kernel, column-major SIMD Givens in bidiagonal QR, and blocked
bidiagonalisation combine to reduce the SVD deficit from $1.86\times$
to $1.59\times$.  The remaining gap is primarily due to the $A^\top A$
eigendecomposition overhead (two extra GEMMs); the direct Golub--Kahan
path (\texttt{svdGKP}) with blocked bidiagonalisation remains
${\sim}2\times$ slower at $100{\times}100$ due to the scalar bidiagonal
QR phase.

\textbf{Double-shift QR (Sub-round 20f):} Implemented but with
negligible measurable impact.  QR iteration accounts for only
$2\text{--}4\%$ of eigenSH time; the dominant cost is
tridiagonalisation ($96\text{--}98\%$).

\textbf{SVD $200{\times}200$:} A regression from $0.98\times$ to
$1.17\times$ is observed.  This is attributed to benchmark run-to-run
variance (high standard deviations of 3--4\,ms on ${\sim}30$\,ms
measurements).

After Round~20, \texttt{linear-massiv} achieves parity or better in
\textbf{all nine operation categories at $500{\times}500$} and in
\textbf{eight of nine at $200{\times}200$ and above}.  The remaining
deficits are SVD at $n \leq 200$ (inherent to the $A^\top A$ pathway)
and eigenSH at $n = 100$ (within noise of parity).

%% ====================================================================
\section{Round~21: D\&C Bidiagonal SVD and Eigensolver Dispatch}
\label{sec:round21}

Round~21 addresses the SVD deficit---the last remaining gap between
\texttt{linear-massiv} and hmatrix/LAPACK---via two complementary
changes to the SVD pipeline.

\subsection{Sub-round 21a: D\&C Bidiagonal SVD for \texttt{svdGKP}}

The bidiagonal QR iteration phase of \texttt{svdGKP} applied $O(n)$
scalar Givens sweeps to full $n{\times}n$ U and V matrices---a pattern
with poor SIMD utilisation.  A divide-and-conquer bidiagonal SVD
(analogous to LAPACK's DBDSDC, following Gu--Eisenstat 1995) replaces
the bidiagonal QR iteration for $n \geq 25$.

\paragraph{Algorithm.}  Given upper bidiagonal $B$ with diagonal
$d[0..n{-}1]$ and superdiagonal $e[0..n{-}2]$:
\begin{enumerate}
\item \textbf{Split} at $k = \lfloor n/2 \rfloor$: save
  $\rho = |e_k|$, zero $e_k$.
\item \textbf{Recurse} on $B_1$ (indices $0..k$) and $B_2$
  (indices $k{+}1..n{-}1$).
\item \textbf{Merge} via a rank-1 secular equation on squared
  singular values: $f(\mu) = 1 + \rho \sum z_i^2/(d_i^2 - \mu) = 0$.
  New singular values $\sigma_{\text{new}} = \sqrt{|\mu|}$.
  V-eigenvectors via Gu--Eisenstat; U-eigenvectors from
  $W_U[j,i] = d_j \cdot W_V[j,i]$ (normalised).  Both local
  accumulators updated via GEMM.
\item \textbf{Base case} ($n \leq 25$): existing bidiagonal QR
  iteration.
\end{enumerate}

\paragraph{Infrastructure reuse.}  The secular equation on
$d^2$ has the identical form as the tridiagonal D\&C eigensolver's
secular equation.  Rather than reimplementing, the D\&C bidiag SVD
reuses \texttt{secularSolve}, \texttt{deflatePartition},
\texttt{dcEigenvectors}, and \texttt{sumZSq} from Symmetric.hs.
These are newly exported for cross-module reuse.

\paragraph{Merge-step GEMM overhead.}  At each merge level, the D\&C
must multiply both local accumulators ($U_{\text{local}}$ and
$V_{\text{local}}$, each $n{\times}n$) by the eigenvector correction
matrix $W$.  This doubles the per-merge GEMM cost compared to the
tridiagonal D\&C (which only has a single $Q$ accumulator).  The term
``merge-step GEMM overhead'' refers to these matrix--matrix multiplies,
not to the GEMM micro-kernel's $k$-loop unrolling (which is a separate
register-pressure issue described in Section~\ref{sec:lessons}).  The
merge-step GEMMs are standard GEMM calls that benefit from the
$4{\times}8$ micro-kernel; the overhead is the \emph{algorithmic cost}
of needing $O(\log n)$ merge levels, each with $O(n^3)$ GEMM work, plus
$O(n^2)$ secular equation and eigenvector computation.  At small $n$,
the secular and eigenvector overhead dominates; at large $n$, the GEMM
work amortises because the SIMD GEMM kernel is highly efficient.

\paragraph{Result.}  The D\&C bidiag SVD improves \texttt{svdGKP}
by ${\sim}20\text{--}25\%$ across all sizes (100, 200, 500), confirming
that converting scalar Givens work into GEMM-dominated computation is
effective.  However, \texttt{svdGKP} remains $2\text{--}3\times$ slower
than \texttt{svdAtAP} because the bidiagonalisation + D\&C pipeline has
higher constant factors than the $A^\top A$ eigendecomposition approach.

\subsection{Sub-round 21b: D\&C Eigensolver Dispatch in \texttt{svdAtAP}}

The larger performance win came from a simple algorithmic dispatch
change.  The \texttt{svdAtAP} path forms $A^\top A$ and computes its
eigendecomposition.  Previously, it used the QR iteration eigensolver
(\texttt{symmetricEigenP}) for all sizes.  Round~21 switches to the
D\&C eigensolver (\texttt{symmetricEigenPDC}) for $n \geq 50$.

This change exploits the fact that the D\&C tridiagonal eigensolver
(stabilised in Round~20d) converts the $O(n^3)$ eigenvector
accumulation from $O(n)$ scalar Givens sweeps into $O(\log n)$ merge
levels dominated by fast GEMM calls.  For the symmetric
eigendecomposition of $A^\top A$, this is the dominant computational
phase after tridiagonalisation.

\paragraph{Result.}  This single dispatch change produced the largest
SVD improvement since Round~6:

\begin{center}
\begin{tabular}{lrrrl}
\toprule
\textbf{Size} & \textbf{hmatrix} & \textbf{lm (R21)} & \textbf{Ratio} & \textbf{vs.\ R20} \\
\midrule
SVD $100{\times}100$ & 3.23\,ms & 4.41\,ms & $\mathbf{1.37\times}$ & $\downarrow$ 1.59 \\
SVD $200{\times}200$ & 25.4\,ms & 22.7\,ms & $\mathbf{0.89\times}$ & $\downarrow$ 1.17 \\
SVD $500{\times}500$ & 346\,ms  & 212\,ms  & $\mathbf{0.61\times}$ & $\downarrow$ 0.77 \\
\bottomrule
\end{tabular}
\end{center}

The SVD $100{\times}100$ deficit shrank from $1.59\times$ to
$1.37\times$ ($14\%$ improvement).  SVD $200{\times}200$ moved from a
$1.17\times$ deficit to $0.89\times$ (now $12\%$ \emph{faster} than
hmatrix/LAPACK).  SVD $500{\times}500$ improved from $0.77\times$ to
$0.61\times$ ($39\%$ faster than hmatrix/LAPACK).

\subsection{Analysis}

\paragraph{Why the D\&C eigensolver helps \texttt{svdAtAP} so much.}
The $A^\top A$ eigendecomposition involves tridiagonalisation followed
by eigenvector computation.  The tridiagonalisation cost is identical
for both QR and D\&C paths.  The difference is in eigenvector
accumulation: QR iteration applies $O(n)$ Givens rotations to the
$n{\times}n$ eigenvector matrix (scalar, strided access), while
D\&C computes eigenvectors via $O(\log n)$ merge levels dominated by
dense GEMM calls to the fast $4{\times}8$ micro-kernel.  Since our
GEMM is $6\text{--}12\times$ faster than OpenBLAS, the GEMM phase
is disproportionately cheap, making D\&C the better choice at $n \geq 50$.

\paragraph{Condition number squaring and numerical quality.}
The D\&C bidiag SVD operates on squared singular values $d_i^2$,
which squares the condition number:
$\kappa^2 = (\sigma_{\max}/\sigma_{\min})^2$.  For ill-conditioned
matrices ($\kappa > 10^8$), significant digits are lost.  The
Gu--Eisenstat formula with log-space computation and perturbation-based
deflation mitigate this for most practical cases.  More fundamentally,
the \texttt{svdAtAP} path \emph{itself} suffers from condition-number
squaring: forming $A^\top A$ gives $\kappa(A^\top A) = \kappa(A)^2$,
so roughly half the significant digits are lost before eigendecomposition
begins.  This is not a new limitation introduced by the D\&C---it is
inherent to the $A^\top A$ approach.  The numerically proper algorithm
avoids forming $A^\top A$ entirely, instead computing the SVD via
orthogonal bidiagonalisation (the \texttt{svdGKP} path).  LAPACK's
\texttt{dgesdd} and \texttt{dgesvd} use this proper approach.  See
Section~\ref{sec:svd-tradeoff} for a detailed discussion of this
performance-vs-quality tradeoff.

After Round~22, \texttt{linear-massiv} achieves parity or better in
\textbf{all nine operation categories at $200{\times}200$ and above},
with the sole remaining deficit being SVD at $100{\times}100$ ($1.38\times$).

\section{Round~22: BLAS-3 Panel Bidiagonalisation and SVD Pipeline Tuning}
\label{sec:round22}

Round~22 applies four complementary optimisations to the SVD pipeline
and its supporting infrastructure.

\subsection{Sub-round 22a: BLAS-3 Panel Bidiagonalisation}

The bidiagonalisation phase of \texttt{svdGKP} previously used a
column-by-column Level-2 approach: each Householder reflector was
applied immediately to the full trailing matrix via rank-1 updates.
This has poor cache behaviour for large matrices because each update
modifies the entire trailing submatrix, evicting useful data from cache.

Round~22 introduces a DLABRD-style panel bidiagonalisation
(GVL4~\S5.4.3) that processes $n_b$ columns at a time.  Within each
panel, left and right Householder reflectors are computed while
maintaining X and Y accumulator matrices that track the deferred
trailing update.  After the panel, the trailing submatrix
$A[k_0{+}n_b{:}m,\; k_0{+}n_b{:}n]$ is corrected via two rank-$n_b$
GEMMs:
\[
  A_{\text{trail}} \mathrel{{-}{=}} V_L \cdot Y^\top + X \cdot V_R^\top
\]
This converts the $O(mn^2)$ bidiagonalisation work from Level-2 (GEMV)
to Level-3 (GEMM) for the trailing update, with only $O(n_b \cdot mn)$
Level-2 work within the panel.  The crossover is set at $n = 128$,
with panel size $n_b = \min(32, \max(8, n/6))$.

\subsection{Sub-round 22b: Tridiagonalisation Parameter Tuning}

The panel tridiagonalisation (DLATRD-style) had conservative thresholds:
panel crossover at $n = 128$ and maximum panel size 48.  Round~22
lowers the crossover to $n = 64$ and raises the maximum panel size to
64.  This enables the Level-3 panel approach for the $100{\times}100$
eigendecomposition in \texttt{svdAtAP} (previously forced to Level-2)
and gives larger panels at $n = 200\text{--}500$ for better GEMM
amortisation.

\subsection{Sub-round 22c: GEMM Packing Crossover}

The GEMM micro-panel packing threshold was lowered from $n = 256$ to
$n = 128$, enabling packed GEMM for medium-sized matrices that appear
in the SVD and eigenvalue pipelines.

\subsection{Sub-round 22d: Pre-Scaled V U-Recovery}

The U-matrix recovery in \texttt{svdAtAP} previously computed
$AV$ via GEMM and then column-scaled by $1/\sigma_j$ in a
separate pass.  Round~22 pre-scales $V$ before the GEMM:
$V_{\text{scaled}}[i,j] = V[i,j] / \sigma_j$, then computes
$U = A \cdot V_{\text{scaled}}$ in a single GEMM.  For square
matrices ($m = n$), the result is written directly into U,
eliminating the intermediate $m{\times}n$ buffer.  The column
scaling uses DoubleX4\# SIMD with a scalar tail loop.
...
\begin{tabular}{lrrrl}
\toprule
\textbf{Size} & \textbf{hmatrix} & \textbf{lm (R22)} & \textbf{Ratio} & \textbf{vs.\ R21} \\
\midrule
SVD $50{\times}50$    & 465\,\textmu s & 805\,\textmu s & $1.73\times$ & $\downarrow$ 1.82 \\
SVD $100{\times}100$  & 2.92\,ms  & 4.02\,ms  & $\mathbf{1.38\times}$ & $\approx$ 1.37 \\
SVD $200{\times}200$  & 24.0\,ms  & 17.6\,ms  & $\mathbf{0.73\times}$ & $\downarrow$ 0.89 \\
SVD $500{\times}500$  & 345\,ms   & 212\,ms   & $\mathbf{0.61\times}$ & $\approx$ 0.61 \\
\bottomrule
\end{tabular}
\end{center}

The largest improvement is at $200{\times}200$: $0.89\times \to 0.73\times$
($18\%$ improvement), now $37\%$ faster than hmatrix/LAPACK.  The
$50{\times}50$ deficit improved from $1.82\times$ to $1.73\times$.
SVD~$100{\times}100$ and $500{\times}500$ held steady.

\subsection{Analysis}

\paragraph{Why the improvement concentrates at $200{\times}200$.}
The DLABRD panel bidiagonalisation (Sub-round 22a) has crossover
$n = 128$: at $200{\times}200$, the bidiagonalisation is dominated by
Level-3 trailing GEMMs ($A_{\text{trail}} \mathrel{{-}{=}} V_L Y^\top
+ X V_R^\top$), each of which exercises the fast $4{\times}8$
micro-kernel.  At $100{\times}100$, the matrix is too small to trigger
the panel path, so the bidiagonalisation remains Level-2.  The
tridiagonalisation parameter tuning (22b) is the primary contributor at
$100{\times}100$: lowering the panel crossover from $128$ to $64$ means
that the eigendecomposition inside \texttt{svdAtAP} now uses the
DLATRD-style panel tridiagonalisation rather than the column-by-column
Level-2 path.  However, at $n = 100$ the trailing update GEMM after each
panel of $\mathtt{nb} \leq 25$ columns operates on a small trailing
submatrix, limiting the GEMM amortisation benefit.

At $500{\times}500$, the bidiagonalisation was already efficient
(dominated by large trailing GEMMs even in the column-by-column path),
so the panel approach gives diminishing returns---the improvement is
absorbed within measurement noise.

\paragraph{The Level-2 to Level-3 conversion theme.}
Round~22a completes the systematic conversion of all major
factorisations from column-by-column Level-2 operations to blocked
Level-3 (GEMM-dominated) trailing updates.  Panel bidiagonalisation
joins panel tridiagonalisation (Round~15), blocked WY Q-accumulation
(Round~17), and panel LU/Cholesky (Round~20e) as the fourth such
conversion.  Section~\ref{sec:lessons} analyses this recurring pattern
in detail.  The remaining unconverted operation is the bidiagonal QR
iteration phase (scalar Givens sweeps), which resists blocking because
each rotation depends on the previous one's result.

\paragraph{Implementation subtleties.}
Two bugs were encountered during panel bidiagonalisation development,
both related to Householder vector storage conventions:
\begin{enumerate}
\item \textbf{Row correction at the panel boundary.}
  When processing the last column of a panel ($k = n{-}2$), no right
  Householder reflector is needed (there is no trailing row to
  eliminate).  However, the \emph{row correction} step---applying the
  accumulated $V_L Y^\top + X V_R^\top$ correction to the current row
  to obtain the correct superdiagonal entry $e[k]$---must still execute.
  The initial implementation gated both the row correction and the right
  Householder behind a single \texttt{k < n - 2} condition, producing
  incorrect bidiagonal entries.

\item \textbf{Implicit Householder leading entries.}
  Right Householder vectors $v_R$ are stored in rows of $A$ to the right
  of the superdiagonal, with an implicit $1$ at position $k{+}1$.
  During the trailing GEMM update, the $V_R^{\top}$ submatrix was read
  directly from $A$, but position $(k, k{+}1)$ contains the
  superdiagonal value $\mu_R$ (from the Householder reduction), not the
  implicit $1$.  This required an explicit conditional in the
  $V_R^{\top}$ construction: entries at the leading position of each
  Householder vector are replaced with $1.0$, entries below are read from
  $A$, and entries above are zero.
\end{enumerate}
Both bugs were caught by running the full test suite with the panel
crossover temporarily set to $n = 4$, forcing the panel path on all
test matrices including $5{\times}5$ and $10{\times}10$ cases.

%% ====================================================================
\section{Round~23: Parameter Tuning and Partial SIMD Secular Evaluation}
\label{sec:round23}

Round~23 implements the eight sub-item optimisations proposed in
\S\ref{sec:future-optimisations} avenues~(a) and~(b) from the Round~22
analysis.  Six parameter changes were applied; two items required no
code changes because they were already active.  A SIMD secular
evaluation was partially implemented.

\subsection{Sub-round 23a: Parameter Tuning}

Eight proposed optimisations were evaluated:

\begin{enumerate}
\item \textbf{Lower panel bidiag crossover $128 \to 64$.}
  Enables BLAS-3 panel bidiagonalisation at $n = 100$ in
  \texttt{svdGKP}, converting Level-2 column-by-column Householder
  updates to Level-3 GEMM trailing updates for the bidiagonalisation
  phase.  At $n = 100$ the panel size is $n_b = \min(32, \max(8,
  100/6)) = 16$, producing ${\sim}6$ panels.

\item \textbf{Increase WY block size $32 \to 48$.}
  The blocked WY accumulation of left and right Householder products
  in \texttt{svdGKP} now uses $n_b = \min(48, n)$ at $n = 100$,
  reducing the number of GEMM calls from 3 to 2 per accumulation.

\item \textbf{Raise D\&C bidiag base case $25 \to 32$.}
  The bidiagonal D\&C base case (below which \texttt{bidiagQRIterP}
  is used) was raised from 25 to 32, allowing the QR iteration
  to operate on slightly larger subproblems where convergence is
  faster relative to the D\&C merge overhead.

\item \textbf{GEMM packing crossover $128 \to 96$.}
  Enables packed-$B$ GEMM for medium-sized matrices ($96 \leq
  \min(m,k,n) < 128$) that appear in eigenvalue and SVD pipelines.

\item \textbf{Tridiag panel size $n/4 \to n/3$.}
  The tridiagonalisation panel size formula was changed from
  $\min(64, \max(16, n/4))$ to $\min(64, \max(16, n/3))$, giving
  $n_b = 33$ at $n = 100$ (3~panels instead of 4) and $n_b = 64$
  at $n \geq 192$.

\item \textbf{D\&C eigensolver for \texttt{svdAtAP} (already active).}
  Investigation revealed that \texttt{svdAtAP} already calls
  \texttt{symmetricEigenPDC} directly for $n \geq 50$, which
  unconditionally uses the D\&C tridiagonal eigensolver.  The
  \texttt{dcCrossover = 1000} guard only applies to the standalone
  \texttt{symmetricEigenP} entry point.  No change needed.

\item \textbf{Sort and column rearrangement (already optimal).}
  The sort-and-permute phase in \texttt{svdAtAP} already uses raw
  \texttt{ByteArray\#} primops with $O(1)$-indexed permutation
  arrays.  No further fusion possible.
\end{enumerate}

\subsection{Sub-round 23b: Partial SIMD Secular Evaluation}

The secular equation functions in the D\&C eigensolver were candidates
for SIMD vectorisation with \texttt{DoubleX4\#}:

\begin{itemize}
\item \texttt{sumZSq}: Sum of $z_i^2$.  Successfully vectorised---the
  inner loop multiplies and accumulates with no division, so there are
  no near-pole hazards.  The SIMD path processes 4~elements per
  iteration with a scalar tail loop.

\item \texttt{secularFuncAndDeriv}: Secular function $f(\mu) = 1 +
  \rho \sum z_i^2 / (d_i - \mu)$ and its derivative.  SIMD attempted
  but \textbf{reverted}: the inner loop divides by $d_i - \mu$, which
  can be near-zero when evaluating near a pole.  The scalar path skips
  these entries (\texttt{if abs diff < 1e-300 then skip}), but SIMD
  cannot branch per lane.  The resulting Inf/NaN values contaminated
  the \texttt{DoubleX4\#} accumulators, causing 4~D\&C eigensolver
  test failures (NaN in 80$\times$80, 100$\times$100, 128$\times$128
  reconstructions).

\item \texttt{farPoleSum}, \texttt{farPoleSumSkip}: Same near-pole
  division issue.  Initially vectorised with a save-and-zero pattern
  to handle the skipped index, but the near-zero denominator problem
  persists for non-skipped entries near the pole.  \textbf{Reverted}
  to scalar after the same 4~test failures.
\end{itemize}

The fundamental obstacle is GHC's lack of per-lane conditional SIMD
operations (e.g., \texttt{blendvpd} on x86).  Without a masked
divide or lane-select primitive, SIMD secular evaluation requires
pre-processing to identify and exclude near-pole entries, which
negates the vectorisation benefit for the small loop counts
($n \leq 100$) typical in D\&C merge steps.
...
\begin{tabular}{lrrrl}
\toprule
\textbf{Operation/Size} & \textbf{hmatrix} & \textbf{lm (R23)} & \textbf{Ratio} & \textbf{vs.\ R22} \\
\midrule
GEMM $100{\times}100$    & 1.38\,ms  & 212\,\textmu s & $0.15\times$ & $\approx$ \\
GEMM $200{\times}200$    & 14.0\,ms  & 1.43\,ms  & $0.10\times$ & $\approx$ \\
GEMM $500{\times}500$    & 225\,ms   & 22.0\,ms  & $0.098\times$ & $\approx$ \\
Dot $n{=}1000$           & 2.34\,\textmu s & 798\,ns & $0.34\times$ & $\approx$ \\
Matvec $100{\times}100$  & 13.1\,\textmu s & 6.63\,\textmu s & $0.51\times$ & $\approx$ \\
LU solve $100{\times}100$ & 422\,\textmu s & 216\,\textmu s & $0.51\times$ & $\approx$ \\
Cholesky $100{\times}100$ & 303\,\textmu s & 160\,\textmu s & $0.53\times$ & $\approx$ \\
QR $100{\times}100$      & 113\,ms   & 2.36\,ms  & $0.021\times$ & $\approx$ \\
eigenSH $100{\times}100$ & 3.29\,ms  & 3.08\,ms  & $0.94\times$ & $\downarrow$ 0.96 \\
eigenSH $200{\times}200$ & 20.5\,ms  & 15.8\,ms  & $0.77\times$ & $\downarrow$ 0.63 \\
eigenSH $500{\times}500$ & 352\,ms   & 173\,ms   & $0.49\times$ & $\approx$ \\
SVD $100{\times}100$     & 4.79\,ms  & 6.09\,ms  & $\mathbf{1.27\times}$ & $\downarrow$ 1.38 \\
SVD $200{\times}200$     & 35.1\,ms  & 30.9\,ms  & $\mathbf{0.88\times}$ & $\uparrow$ 0.73 \\
SVD $500{\times}500$     & 616\,ms   & 334\,ms   & $\mathbf{0.54\times}$ & $\downarrow$ 0.61 \\
\bottomrule
\end{tabular}
\end{center}
...
\paragraph{SVD $100{\times}100$ improved from $1.38\times$ to $1.27\times$.}
The combined parameter changes---panel bidiag crossover lowered to 64,
WY block size increased to 48, D\&C base case raised to 32, and
GEMM packing crossover lowered to 96---produce a modest $8\%$
improvement at $100{\times}100$.  The largest contributor is the panel
bidiag crossover reduction, which enables Level-3 bidiagonalisation
for \texttt{svdGKP} at $n = 100$.  However, the panel size ($n_b = 16$)
is small relative to the matrix, limiting GEMM amortisation.

\paragraph{SVD $500{\times}500$ improved from $0.61\times$ to $0.54\times$.}
The GEMM packing crossover reduction (96) and tridiag panel size
increase ($n/3$) contribute to a $12\%$ improvement at $500{\times}500$.
At this size, the panel parameters produce larger trailing GEMMs that
better utilise the $4{\times}8$ micro-kernel.

\paragraph{SVD $200{\times}200$ regressed from $0.73\times$ to $0.88\times$.}
This regression is unexpected and warrants investigation.
Benchmark-to-benchmark variance (the $200{\times}200$ measurements
show 8--10\% standard deviation) may account for part of it.
The tridiag panel size change ($n/4 \to n/3$) produces $n_b = 64$
at $n = 200$, which gives only 2~panels---the second panel operates
on a very small trailing matrix, potentially reducing overall efficiency
compared to the 3--4 smaller panels at $n_b = 50$.

\paragraph{eigenSH $200{\times}200$ regressed from $0.63\times$ to $0.77\times$.}
The tridiag panel size change affects eigenSH directly.  At $n = 200$,
$n_b = 64$ (from $n/3$) vs.\ the previous $n_b = 50$ (from $n/4$)
produces fewer but larger panels.  The larger panel size means more
Level-2 work \emph{within} each panel relative to the Level-3 trailing
update, which may explain the regression.  The effect reverses at
$n = 500$ where $n_b = 64$ was already the cap under both formulae.

\paragraph{SIMD secular evaluation: a cautionary lesson.}
The attempted SIMD vectorisation of the secular equation functions
illustrates a fundamental limitation of GHC's current SIMD model:
the absence of per-lane conditional operations makes it impossible
to vectorise loops containing data-dependent branches without
pre-processing.  For the secular equation, the branch frequency
(near-pole entries) is low but the consequence (Inf/NaN contamination)
is catastrophic.  Only \texttt{sumZSq}, which has no division, could
be safely vectorised.  Future GHC SIMD improvements (masked
operations, per-lane select) would unlock the remaining functions.
...
Table~\ref{tab:final-results} presents the final Round~23 benchmark
...
(OpenBLAS/LAPACK), Round~23.  Ratios $< 1$ indicate \texttt{linear-massiv}
is faster.}
...
\midrule
GEMM $100{\times}100$       & 1.38\,ms  & 212\,\textmu s & $0.15\times$ & $6.5\times$ \\
GEMM $200{\times}200$       & 14.0\,ms  & 1.43\,ms       & $0.10\times$ & $9.8\times$ \\
GEMM $500{\times}500$       & 225\,ms   & 22.0\,ms       & $0.098\times$ & $10.2\times$ \\
Dot product $n{=}1000$      & 2.34\,\textmu s & 798\,ns & $0.34\times$ & $2.9\times$ \\
Matvec $100{\times}100$     & 13.1\,\textmu s & 6.63\,\textmu s & $0.51\times$ & $2.0\times$ \\
LU solve $100{\times}100$   & 422\,\textmu s & 216\,\textmu s & $0.51\times$ & $1.95\times$ \\
Cholesky $100{\times}100$   & 303\,\textmu s & 160\,\textmu s & $0.53\times$ & $1.89\times$ \\
QR $100{\times}100$         & 113\,ms   & 2.36\,ms       & $0.021\times$ & $47.8\times$ \\
eigenSH $100{\times}100$    & 3.29\,ms  & 3.08\,ms       & $0.94\times$ & $1.07\times$ \\
eigenSH $200{\times}200$    & 20.5\,ms  & 15.8\,ms       & $0.77\times$ & $1.30\times$ \\
eigenSH $500{\times}500$    & 352\,ms   & 173\,ms        & $0.49\times$ & $2.03\times$ \\
SVD $100{\times}100$        & 4.79\,ms  & 6.09\,ms       & $\mathbf{1.27\times}$ & --- \\
SVD $200{\times}200$        & 35.1\,ms  & 30.9\,ms       & $\mathbf{0.88\times}$ & $1.14\times$ \\
SVD $500{\times}500$        & 616\,ms   & 334\,ms        & $\mathbf{0.54\times}$ & $1.84\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
operation categories}.  Round~23's parameter tuning reduced the SVD
$100{\times}100$ deficit from $1.38\times$ to $1.27\times$ ($8\%$
improvement) and SVD $500{\times}500$ from $0.61\times$ to
$0.54\times$ ($1.84\times$ faster than LAPACK).  SVD $200{\times}200$
showed some regression ($0.73\times \to 0.88\times$), likely due to
benchmark variance.  The sole remaining deficit is SVD at
$100{\times}100$ ($1.27\times$).
...
\small
\begin{tabular}{l@{\quad}r@{\quad}r@{\quad}r@{\quad}r@{\quad}r@{\quad}r@{\quad}r@{\quad}r@{\quad}r@{\quad}r@{\quad}r}
\toprule
\textbf{Operation} & \textbf{R1} & \textbf{R3} & \textbf{R6} & \textbf{R10} & \textbf{R17} & \textbf{R18} & \textbf{R19} & \textbf{R20} & \textbf{R21} & \textbf{R22} & \textbf{R23} \\
\midrule
GEMM $100{\times}100$    & $329\times$     & $0.50\times$ & $0.50\times$ & ---          & $0.16\times$ & $0.15\times$ & $0.15\times$ & $0.17\times$ & $=$ & $=$ & $=$ \\
LU solve $100{\times}100$ & $295\times$    & ---          & $0.37\times$ & $0.37\times$ & $0.68\times$ & $0.68\times$ & $0.68\times$ & $0.47\times$ & $=$ & $=$ & $=$ \\
Cholesky $100{\times}100$ & $240\times$    & ---          & $0.34\times$ & $0.34\times$ & $0.63\times$ & $0.48\times$ & $0.48\times$ & $0.51\times$ & $=$ & $=$ & $=$ \\
QR $100{\times}100$       & ${\approx}260\times$ & ---    & $0.13\times$ & ---          & $0.022\times$& $0.021\times$ & $0.021\times$ & $0.020\times$ & $=$ & $=$ & $=$ \\
eigenSH $100{\times}100$  & $897\times$    & ---          & $0.93\times$ & $0.97\times$ & $1.04\times$ & $1.18\times$ & $1.00\times$ & $0.96\times$ & $=$ & $=$ & $0.94\times$ \\
eigenSH $200{\times}200$  & ---            & ---          & ---          & ---          & ---          & $1.19\times$ & $0.69\times$ & $0.63\times$ & $=$ & $=$ & $0.77\times$ \\
SVD $100{\times}100$      & $887\times$    & ---          & $3.28\times$ & $2.38\times$ & $1.80\times$ & $1.78\times$ & $1.86\times$ & $1.59\times$ & $1.37\times$ & $1.38\times$ & $\mathbf{1.27\times}$ \\
SVD $200{\times}200$      & ---            & ---          & ---          & ---          & ---          & $1.53\times$ & $0.98\times$ & $1.17\times$ & $0.89\times$ & $0.73\times$ & $\mathbf{0.88\times}$ \\
eigenSH $500{\times}500$  & ---            & ---          & ---          & ---          & $0.51\times$ & $0.47\times$ & $0.48\times$ & $0.49\times$ & $=$ & $=$ & $=$ \\
SVD $500{\times}500$      & ---            & ---          & ---          & ---          & $0.90\times$ & $0.76\times$ & $0.82\times$ & $0.77\times$ & $0.61\times$ & $0.61\times$ & $\mathbf{0.54\times}$ \\
\bottomrule
\end{tabular}
\end{table}

The trajectory reveals eight distinct phases:
\begin{enumerate}
\item \textbf{Rounds~2--3 (ByteArray\# breakthrough):} Raw primop kernels
  ...
  single step, achieving $2\times$ faster-than-hmatrix/LAPACK GEMM.
...
\item \textbf{Rounds~7--18 (constant-factor refinement):} SIMD inner
  ...
  blocked WY representations pushed remaining categories past hmatrix/LAPACK.
...
\item \textbf{Round~20 (algorithmic deepening):} Fused DSYRK, blocked
  bidiagonalisation, D\&C eigensolver stabilisation, panel
  LU/Cholesky factorisation, double-shift QR, GEMM micro-panel packing,
  and 2D parallel GEMM tiling.  LU improved from $0.68\times$ to
  $0.47\times$ ($2.1\times$ faster than hmatrix/LAPACK); GEMM at~500
  from $0.092\times$ to $0.084\times$ ($11.9\times$ faster) via
  packing; SVD deficit reduced from $1.86\times$ to $1.59\times$
  at~100; eigenSH passed parity at~100 ($0.96\times$) and reached
  $0.49\times$ at~500.
\item \textbf{Round~21 (D\&C SVD breakthrough):} D\&C bidiagonal SVD
  for the \texttt{svdGKP} path (${\sim}20\%$ improvement), and---more
  significantly---switching \texttt{svdAtAP}'s eigendecomposition from
  QR iteration to the D\&C eigensolver for $n \geq 50$.  SVD deficit at
  $100{\times}100$ reduced from $1.59\times$ to $1.37\times$; SVD
  $200{\times}200$ moved from $1.17\times$ deficit to $0.89\times$
  (faster than LAPACK); SVD $500{\times}500$ from $0.77\times$ to
  $0.61\times$.  All nine categories now faster at $200{\times}200$.
\item \textbf{Round~22 (BLAS-3 pipeline tuning):} DLABRD-style panel
  bidiagonalisation for \texttt{svdGKP}, tridiagonalisation parameter
  tuning (crossover $128 \to 64$, max panel $48 \to 64$), GEMM packing
  crossover lowered ($256 \to 128$), and pre-scaled V U-recovery for
  \texttt{svdAtAP}.  SVD $200{\times}200$ improved from $0.89\times$
  to $0.73\times$ ($37\%$ faster than LAPACK); SVD $50{\times}50$
  from $1.82\times$ to $1.73\times$.
\item \textbf{Round~23 (parameter tuning and partial SIMD):} Systematic
  application of eight proposed optimisations from the Round~22
  analysis: panel bidiag crossover $128 \to 64$, WY block size
  $32 \to 48$, D\&C bidiag base case $25 \to 32$, GEMM packing
  crossover $128 \to 96$, tridiag panel size $n/4 \to n/3$, and
  SIMD \texttt{sumZSq}.  SVD $100{\times}100$ deficit reduced from
  $1.38\times$ to $1.27\times$; SVD $500{\times}500$ from
  $0.61\times$ to $0.54\times$ ($1.84\times$ faster than LAPACK).
  SIMD secular evaluation attempted for three additional functions
  but reverted due to GHC's lack of per-lane conditional SIMD.
\end{enumerate}
...
The twenty-three rounds of optimisation yield several cross-cutting
...
achieving ${\sim}12\times$ faster-than-OpenBLAS GEMM at $500{\times}500$.
...
divide-and-conquer (D\&C) for eigenvalue problems.  Through Round~20,
QR iteration consistently outperformed D\&C at all benchmarked sizes
($n \leq 500$), for three reasons:
...
\paragraph{Round~21 reversal.}  However, Round~21 revealed that D\&C
is \emph{strongly preferred} when used as the eigensolver within
\texttt{svdAtAP}'s $A^\top A$ eigendecomposition pathway.  The key
insight is that the merge-step GEMM overhead, which penalises D\&C in
standalone \texttt{symmetricEigenPDC} benchmarks, becomes an
\emph{advantage} in the SVD context because \texttt{linear-massiv}'s
GEMM kernel is $6\text{--}12\times$ faster than OpenBLAS.  The D\&C
converts $O(n)$ scalar Givens sweeps (which SIMD vectorises poorly)
into $O(\log n)$ dense GEMM calls (which the $4{\times}8$
micro-kernel handles extremely efficiently).  This produced a
$37\%$ reduction in SVD $100{\times}100$ time and a $21\%$ reduction
at $500{\times}500$, making it the most impactful SVD optimisation
since Round~6.  The lesson: algorithmic choice depends not just on
asymptotic complexity but on the \emph{relative efficiency} of the
sub-operations on the target platform.

\subsection{The Level-2 to Level-3 Conversion}

A systematic optimisation pattern that emerged across Rounds~15--22
is the conversion of column-by-column Level-2 BLAS operations into
blocked Level-3 (GEMM-dominated) operations.  This is the same
strategy that underlies LAPACK's high performance---routines like
\texttt{DGEBRD} (bidiagonalisation), \texttt{DGETRF} (LU), and
\texttt{DLATRD} (tridiagonalisation) all use panel factorisation
with deferred trailing updates---but implementing it in pure Haskell
required solving each case's Householder storage conventions and
implicit-entry bookkeeping individually.

The pattern is always the same:
\begin{enumerate}
\item Process $n_b$ columns (a \emph{panel}) using Level-2 operations
  within the panel, recording the deferred effect on the trailing
  submatrix in accumulator matrices (X, Y, or T-factor).
\item After the panel, apply the cumulative correction to the trailing
  submatrix via one or two rank-$n_b$ GEMMs.
\end{enumerate}
The total arithmetic work is unchanged, but the trailing GEMM achieves
far higher arithmetic intensity---the $4{\times}8$ micro-kernel sustains
${\sim}32$ flops per cache line versus ${\sim}4$ for a rank-1 GEMV
update---and the panel's Level-2 work is confined to a narrow
$m{\times}n_b$ strip that fits in L1/L2 cache.

Table~\ref{tab:level3-conversion} summarises the conversions.

\begin{table}[h]
\centering
\caption{Level-2 to Level-3 conversions across rounds.}
\label{tab:level3-conversion}
\small
\begin{tabular}{llll}
\toprule
\textbf{Round} & \textbf{Operation} & \textbf{LAPACK analogue} & \textbf{Impact} \\
\midrule
15 & Tridiagonalisation  & \texttt{DLATRD} & eigenSH $1.05\times$ at 100 \\
17 & QR Q-accumulation   & \texttt{DLARFT}/\texttt{DLARFB} & QR $0.022\times$ \\
20e & LU factorisation   & \texttt{DGETRF} & LU $0.47\times$ \\
20e & Cholesky            & \texttt{DPOTRF} & Cholesky $0.51\times$ \\
22a & Bidiagonalisation  & \texttt{DGEBRD}/\texttt{DLABRD} & SVD 200 $0.73\times$ \\
\bottomrule
\end{tabular}
\end{table}

The one operation that resists this conversion is the bidiagonal/tridiagonal
QR iteration phase: each Givens rotation depends on the previous one's
output, preventing batching.  The D\&C approach (Rounds~20d, 21) addresses
this differently, replacing the sequential Givens sweeps with
GEMM-dominated merge steps---a complementary strategy that achieves
the same goal of shifting work from scalar to SIMD-friendly operations.

\subsection{Numerical Quality vs.\ Performance: The SVD Tradeoff}
\label{sec:svd-tradeoff}

The most widely-accepted SVD algorithm---used by LAPACK's
\texttt{dgesdd} and \texttt{dgesvd}---avoids forming $A^\top A$
entirely.  Instead, it reduces $A$ to upper bidiagonal form $B$ via
orthogonal Householder reflections ($A = U_0 B V_0^\top$), then
computes the SVD of $B$ directly.  Because every transformation is
orthogonal, the condition number is preserved throughout:
$\kappa(B) = \kappa(A)$.  This is \texttt{svdGKP} in
\texttt{linear-massiv}.

The faster \texttt{svdAtAP} path takes the numerically inferior
approach: it forms $A^\top A$ explicitly and eigendecomposes it.
This \emph{squares} the condition number---$\kappa(A^\top A) =
\kappa(A)^2$---so roughly half the significant digits are lost before
the eigendecomposition even begins.  For a matrix with
$\kappa(A) = 10^8$ (not uncommon in scientific computing),
$\kappa(A^\top A) = 10^{16}$, which exceeds double-precision's
${\sim}15.9$ decimal digits, leaving essentially no accuracy in the
small singular values.

The performance hierarchy is therefore inverted relative to the
numerical quality hierarchy:

\begin{center}
\begin{tabular}{lcc}
\toprule
\textbf{Path} & \textbf{Numerical quality} & \textbf{Performance} \\
\midrule
\texttt{svdGKP} (Golub--Kahan) & Best ($\kappa$ preserved) & Slower ($2\text{--}3\times$ at 100) \\
\texttt{svdAtAP} ($A^\top A$ eigen) & Worst ($\kappa^2$) & Faster (default) \\
LAPACK \texttt{dgesdd} & Best ($\kappa$ preserved) & Reference \\
\bottomrule
\end{tabular}
\end{center}

LAPACK achieves both numerical quality \emph{and} performance because
its Golub--Kahan path has been optimised for decades with highly-tuned
Fortran BLAS-3 bidiagonalisation routines (\texttt{DGEBRD},
\texttt{DBDSDC}).  The remaining $1.27\times$ SVD deficit at
$100{\times}100$ therefore reflects two compounding factors:
(a)~\texttt{svdAtAP}'s $A^\top A$ approach is faster but numerically
inferior, and (b)~the numerically proper \texttt{svdGKP} path has not
yet reached LAPACK's level of bidiagonalisation micro-optimisation.

\paragraph{Practical implications.}
For well-conditioned problems ($\kappa < 10^6$), the $A^\top A$
pathway loses at most a few digits and is acceptable.  For
ill-conditioned problems, users should prefer \texttt{svdGKP} despite
its higher cost, or use iterative refinement.  A future default
dispatch could select the path based on a cheap condition-number
estimate (e.g.\ from the diagonal of $R$ in a QR factorisation of $A$).
...
\subsection{The AVX SIMD Extension Family}
\label{sec:avx-family}

All SIMD kernels in \texttt{linear-massiv} use GHC's
\texttt{DoubleX4\#} primops, which map to AVX2 256-bit vector
instructions via the LLVM~17 backend.  The AVX family of x86 SIMD
extensions forms a capability hierarchy:

\begin{description}
\item[SSE2 (2001):] 128-bit XMM registers (2~doubles), baseline for
  x86-64.  All 64-bit x86 processors support SSE2.
\item[AVX (2011):] Widens registers to 256-bit YMM (4~doubles),
  adds 3-operand non-destructive encoding.  Sandy Bridge onwards.
\item[AVX2 (2013):] Extends 256-bit operations to integer types
  (the original AVX was floating-point only).  Haswell onwards.
  This is \texttt{linear-massiv}'s compilation target.
\item[FMA (2013):] Fused multiply-add instructions
  (\texttt{vfmadd231pd}) that compute $a \cdot b + c$ in a single
  cycle with one rounding.  Critical for GEMM performance.  Haswell
  onwards (co-introduced with AVX2).
\item[AVX-512 (2017):] 512-bit ZMM registers (8~doubles), with 32
  registers (vs.\ 16 YMM).  Xeon Scalable (Skylake-SP) and AMD Zen~4
  onwards.  Subdivided into many sub-extensions (F, CD, BW, DQ, VL,
  VNNI, BF16, etc.)\ with varying processor support.
\end{description}

\paragraph{Relevance to this project.}
The development machine (Intel~i7-1370P, Raptor Lake) supports AVX2
and FMA but \emph{not} AVX-512.  Intel removed AVX-512 from hybrid
(P-core + E-core) architectures starting with Alder Lake (12th~gen),
fusing it off at the hardware level even on P-cores.  Pinning threads
to P-cores does not restore AVX-512 on these processors.  AVX-512
would only benefit deployment on Intel Xeon Scalable, AMD Zen~4+, or
similar server-class hardware.

\subsection{Type System Design: Massiv vs.\ Vector-Sized}
\label{sec:type-system-design}

\texttt{linear-massiv}'s core types are phantom-typed newtypes over
massiv's \texttt{Array} type:

\begin{verbatim}
newtype Matrix m n r e = MkMatrix { unMatrix :: Array r Ix2 e }
newtype Vector n   r e = MkVector { unVector :: Array r Ix1 e }
\end{verbatim}

The type-level naturals \texttt{m}, \texttt{n} (via \texttt{KnownNat})
provide compile-time dimensional safety: dimension mismatches are
type errors.  The representation parameter \texttt{r} selects massiv's
computation strategy (\texttt{M.P} for parallel, \texttt{M.D} for
delayed/fused, etc.).

An alternative would be to use the \texttt{vector-sized} package,
which provides a length-indexed wrapper around \texttt{Data.Vector}.
This was evaluated and rejected for several reasons:

\begin{enumerate}
\item \textbf{Multi-dimensional indexing.}  Matrices require 2D
  indexing (\texttt{Ix2}).  \texttt{vector-sized} provides only 1D
  vectors; a custom 2D layer would lose massiv's optimised stride
  calculations.
\item \textbf{Computation strategies.}  massiv's delayed arrays
  (\texttt{M.D}, \texttt{M.DW}) enable fusion of element-wise
  operations, and its parallel strategy (\texttt{M.P}) integrates
  with the \texttt{scheduler} package for work-stealing parallelism.
  \texttt{vector-sized} wraps flat vectors with no equivalent.
\item \textbf{Hot-path irrelevance.}  The numerical kernels operate
  on raw \texttt{MutableByteArray\#} via GHC primops.  Neither
  massiv's \texttt{Array} wrapper nor \texttt{vector-sized}'s
  \texttt{Vector} wrapper appears on the critical path---both are
  erased at compile time.  Adding another wrapper layer provides no
  performance or safety benefit.
\item \textbf{Type-level approach is identical.}  Both
  \texttt{vector-sized} and \texttt{linear-massiv} use
  \texttt{KnownNat} constraints for length safety.  There is no
  additional type safety to gain from switching.
\end{enumerate}

The current design provides the best of both worlds: type-safe
dimensions at the API level, massiv's parallel/fused computation
strategies for non-hot-path operations, and raw \texttt{ByteArray\#}
performance in the inner kernels.

%% ====================================================================
\section{When to Use Each Library}
\label{sec:when-to-use}

The twenty-two rounds of optimisation have substantially changed the
...
\item[\texttt{hmatrix}] Still relevant for SVD at $n \leq 100$, where
  ...
  ($1.38\times$ at $100{\times}100$).  At $200{\times}200$ and above,
  \texttt{linear-massiv} is now faster ($0.73\times$).
  Also appropriate when existing codebases depend on its API, or when the
  full breadth of LAPACK routines (sparse solvers, banded systems,
  generalised eigenproblems) is needed---operations that
  \texttt{linear-massiv} does not yet implement.

\item[\texttt{linear-massiv}] Now the performance leader in \textbf{all
  nine benchmarked categories at $200{\times}200$ and above}, and in
  eight of nine at $100{\times}100$.  EigenSH is $1.04\times$ faster at
  $100{\times}100$ and $1.6\times$ faster at $200{\times}200$; SVD is
  $1.4\times$ faster at $200{\times}200$ and $1.6\times$ faster at
  $500{\times}500$; LU solve is $2.1\times$ faster at $100{\times}100$.
  ...
  hmatrix/LAPACK for raw numerical throughput.
\end{description}
...
After twenty-three rounds of optimisation, \texttt{linear-massiv}
outperforms hmatrix (OpenBLAS/LAPACK) in all nine categories at
$500{\times}500$ and in eight of nine at $200{\times}200$.
Rounds~20--23 implemented all proposed optimisations from the
post-Round~19 roadmap, plus D\&C bidiagonal SVD, D\&C eigensolver
dispatch, BLAS-3 panel bidiagonalisation, parameter tuning, and
partial SIMD secular evaluation.
The sole remaining deficit is SVD at $100{\times}100$ ($1.27\times$).

The roadmap below is revised to reflect the post-Round~23 state.
Items~1--7 from the original roadmap are implemented
(Section~\ref{sec:round20}); D\&C bidiagonal SVD and eigensolver
dispatch in Section~\ref{sec:round21}; BLAS-3 panel bidiagonalisation
and pipeline tuning in Section~\ref{sec:round22}; parameter tuning
and partial SIMD in Section~\ref{sec:round23}.  Remaining
opportunities focus on infrastructure scaling, future GHC capabilities,
and the fundamental SIMD limitation encountered in Round~23.

%% ------------------------------------------------------------------
\subsection{Implemented in Round~20}

The following items from the previous roadmap have been completed:

\begin{enumerate}
\item \textbf{Blocked Golub--Kahan SVD} (Section~\ref{sec:round20},
  Sub-rounds 20a--20c): Fused DSYRK, column-major SIMD Givens in
  bidiagonal QR, blocked bidiagonalisation with WY accumulation.
  SVD $100{\times}100$ improved from $1.86\times$ to $1.59\times$.
\item \textbf{D\&C eigensolver stabilisation} (Sub-round 20d):
  Perturbation-based deflation, fixed-pole separation, close-$d$
  Givens deflation.  Reconstruction error reduced from $166\text{--}573$
  to $< 5 \times 10^{-7}$ at $n = 128$.  Crossover set to $n = 1000$.
\item \textbf{Fused DSYRK kernel} (Sub-round 20a):
  \texttt{rawSyrkLowerKernel} computes $A^\top A$ with half the flops
  and one allocation instead of two.
\item \textbf{Panel LU and Cholesky} (Sub-round 20e): Panel
  factorisation with $\mathtt{nb} = 32$ and GEMM trailing updates.
  LU improved from $0.68\times$ to $0.47\times$.
\item \textbf{Double-shift QR} (Sub-round 20f): Implicit double-shift
  bulge chase for symmetric tridiagonal QR iteration.  Implemented but
  with negligible measurable impact (QR iteration is only
  $2\text{--}4\%$ of eigenSH time).
\item \textbf{GEMM micro-panel packing} (Sub-round 20g):
  $bk$-outer loop with packed $B$ micro-panels for
  $\min(m,k,n) \geq 256$.  GEMM $500{\times}500$ improved from
  $0.092\times$ to $0.084\times$ ($8.7\%$).
\item \textbf{2D parallel GEMM tiling} (Sub-round 20h): Column-restricted
  kernel with 2D grid partitioning for $\geq 4$ threads.  Reduces
  per-thread $B$ working set by $\sqrt{p}$.  No impact on
  single-threaded benchmarks.
\end{enumerate}

%% ------------------------------------------------------------------
\subsection{Implemented in Rounds~21--23}

Beyond the Round~20 roadmap items, Rounds~21--23 addressed the SVD
deficit with seven additional optimisations:

\begin{enumerate}
\setcounter{enumi}{7}
\item \textbf{D\&C bidiagonal SVD for \texttt{svdGKP}}
  (Section~\ref{sec:round21}, Sub-round 21a): Replaces bidiagonal QR
  iteration with divide-and-conquer using the Gu--Eisenstat secular
  equation solver (reused from the tridiagonal D\&C).
  \texttt{svdGKP} improved by ${\sim}20\text{--}25\%$ at all sizes.
\item \textbf{D\&C eigensolver dispatch in \texttt{svdAtAP}}
  (Sub-round 21b): Switching from QR iteration to D\&C for the
  $A^\top A$ eigendecomposition at $n \geq 50$.  The single most
  impactful SVD change since Round~6: SVD $100{\times}100$ from
  $1.59\times$ to $1.37\times$; $200{\times}200$ from $1.17\times$
  to $0.89\times$; $500{\times}500$ from $0.77\times$ to
  $0.61\times$.
\item \textbf{BLAS-3 panel bidiagonalisation}
  (Section~\ref{sec:round22}, Sub-round 22a): DLABRD-style blocked
  bidiagonalisation with X/Y accumulators and rank-$n_b$ GEMM trailing
  updates.  Primary impact at $200{\times}200$: $0.89\times \to
  0.73\times$.
\item \textbf{SVD pipeline tuning} (Sub-rounds 22b--22d):
  Tridiagonalisation crossover $128 \to 64$, GEMM packing crossover
  $256 \to 128$, pre-scaled V U-recovery with DoubleX4\# SIMD.
  Incremental gains across sizes.
\item \textbf{Parameter tuning} (Section~\ref{sec:round23}, Sub-round
  23a): Panel bidiag crossover $128 \to 64$, WY block size $32 \to 48$,
  D\&C bidiag base case $25 \to 32$, GEMM packing crossover
  $128 \to 96$, tridiag panel size $n/4 \to n/3$.  SVD
  $100{\times}100$ deficit from $1.38\times$ to $1.27\times$; SVD
  $500{\times}500$ from $0.61\times$ to $0.54\times$.
\item \textbf{SIMD \texttt{sumZSq}} (Sub-round 23b): DoubleX4\#
  vectorisation of the sum-of-squares function used in the D\&C
  secular equation solver.  The division-free inner loop avoids the
  near-pole hazard that blocked SIMD for the other secular functions.
\item \textbf{SIMD secular evaluation (attempted, partially reverted)}
  (Sub-round 23b): SIMD vectorisation of \texttt{secularFuncAndDeriv},
  \texttt{farPoleSum}, and \texttt{farPoleSumSkip} was implemented but
  reverted due to GHC's lack of per-lane conditional SIMD
  (Section~\ref{sec:round23}).
\end{enumerate}

%% ------------------------------------------------------------------
\subsection{1.\ GEMM Micro-Panel Packing}
\label{sec:future-gemm-packing}

\paragraph{Status: implemented in Round~20 (Sub-round 20g).}
The packed variant \texttt{rawGemmBISlicePackedBK} (Kernel.hs) is
dispatched for $\min(m,k,n) \geq 96$ via \texttt{gemmPackCrossover}
(lowered from 256 to 128 in Round~22c, then to 96 in Round~23a).

\paragraph{Implementation.}
The $bk$-outer loop packs the current $B$ tile (up to $64 \times n$)
into 8-column micro-panels in panel-major layout (stride~8), then
freezes via \texttt{unsafeFreezeByteArray\#} to enable pure
\texttt{indexDoubleArrayAsDoubleX4\#} reads in the $k$-loop.  A
critical finding: State\# threading through a mutable-read $k$-loop
prevents GHC from fully unboxing accumulator chains, causing a $6.5\%$
regression.  The freeze-then-pure-read approach avoids this, yielding
an $8.7\%$ improvement over the unpacked path at $500{\times}500$.

\paragraph{Measured impact.}
GEMM $500{\times}500$: $0.092\times \to 0.084\times$ ($11.9\times$
faster than OpenBLAS).  Below the crossover ($n < 256$), no change.
The improvement cascades to eigenSH ($0.54\times \to 0.49\times$
at~500) and SVD ($0.81\times \to 0.77\times$ at~500) via internal
GEMM calls.

\paragraph{Remaining opportunity.}
$A$-panel packing (column-contiguous for broadcast) is not yet
implemented.  This would further reduce TLB pressure at $n > 1000$.
The crossover was further lowered to $96$ in Round~23a, enabling packed
GEMM for the medium-sized internal GEMMs in the SVD/eigenvalue
pipelines.

%% ------------------------------------------------------------------
\subsection{2.\ 2D Parallel GEMM Tiling}
\label{sec:future-2d-gemm}

\paragraph{Status: implemented in Round~20 (Sub-round 20h).}
The column-restricted kernel \texttt{rawGemmBIBJSlice} (Kernel.hs)
and 2D grid dispatch in \texttt{matMulPPar} (Level3.hs) are active
for $\mathtt{numThreads} \geq 4$ and $m, n \geq 128$.

\paragraph{Implementation.}
A new \texttt{rawGemmBIBJSlice} kernel accepts $bjStart$/$bjEnd$
parameters, enabling each thread $(t_i, t_j)$ to compute only its
assigned row and column tile of $C$.  Grid dimensions are
$\lfloor\sqrt{p}\rfloor \times \lceil p/\lfloor\sqrt{p}\rfloor\rceil$,
reducing per-thread $B$ working set by $\sqrt{p}$.

\paragraph{Measured impact.}
No impact on single-threaded benchmarks (the primary comparison
target).  Parallel GEMM scaling is improved for $\geq 4$ threads on
matrices $\geq 128{\times}128$.

%% ------------------------------------------------------------------
\subsection{3.\ Cache Tile and Panel Size Auto-Tuning}
\label{sec:future-autotune}

\paragraph{Status: implementable now.}  No external blockers; deferred
because the development machine's cache hierarchy is the only one
benchmarked, so the current hard-coded sizes are already optimal for
the measured results.  Would only improve performance on different
hardware (Apple M-series, Intel Golden Cove, etc.).

\paragraph{Current state.}
GEMM tile size $\mathtt{bs} = 64$ (Kernel.hs) and tridiagonalisation
panel size $\mathtt{nb} = \min(64, \max(16, n/3))$ (Symmetric.hs, tuned
in Rounds~22b and~23a) are tuned on AMD Zen~4 (32\,KB L1d, 512\,KB L2).  These are
...
%% ------------------------------------------------------------------
\subsection{4.\ AVX-512 Support via \texttt{DoubleX8\#}}
\label{sec:future-avx512}

\paragraph{Status: implementable with caveats.}  Contrary to earlier
assumptions, GHC has defined \texttt{DoubleX8\#} as a primitive SIMD
type since GHC~7.8, alongside the full complement of 512-bit vector
types (\texttt{FloatX16\#}, \texttt{Int64X8\#}, etc.).  These primops
are \emph{usable today} when compiling with the LLVM backend
(\texttt{-fllvm}) and the \texttt{-mavx512f} flag, which instructs
LLVM to generate AVX-512 instructions using ZMM registers.
The \textbf{native code generator} (NCG), however, does \textbf{not}
support 512-bit vectors: GHC~9.12 added initial 128-bit NCG SIMD
support (MR~!12860), with 256-bit and 512-bit NCG support explicitly
listed as future work.  Since \texttt{linear-massiv} already uses the
LLVM backend (\texttt{-fllvm -mavx2 -mfma}), switching to
\texttt{-mavx512f} and using \texttt{DoubleX8\#} is technically
feasible.

The remaining obstacles are practical rather than fundamental:
\begin{enumerate}
\item \textbf{Register pressure.}  The current $4{\times}8$
  GEMM micro-kernel holds ${\sim}14$ simultaneous \texttt{DoubleX4\#}
  values, near the LLVM register allocator's spill threshold.  With
  ZMM registers, an $8{\times}8$ micro-kernel would require ${\sim}14$
  simultaneous \texttt{DoubleX8\#} values (8 accumulators +
  $B$-vectors + $A$-broadcasts).  Whether LLVM's register allocator
  handles this without spilling is untested.
\item \textbf{AVX-512 availability.}  Not all target CPUs support
  AVX-512 (absent on consumer AMD Zen~2/3, Apple Silicon, many
  laptops).  A build-time or runtime dispatch mechanism would be
  needed to provide both AVX2 and AVX-512 code paths.
\item \textbf{Thermal throttling.}  Some Intel processors reduce clock
  frequency when executing AVX-512 instructions, partially or fully
  negating the wider vector benefit.
\end{enumerate}
...
\paragraph{Proposed optimisation.}
Provide alternative 8-wide micro-kernels using \texttt{DoubleX8\#}
primops, compiled with \texttt{-fllvm -mavx512f}:
\begin{itemize}
...
\item Compile-time selection via CPP based on target architecture,
  or build-time flag (\texttt{-favx512}) to select the 512-bit
  code path.
\end{itemize}
...
contingent on LLVM register allocator behaviour with 512-bit types
and absence of thermal throttling on the target CPU.  The current
register pressure limit (${\sim}14$ simultaneous \texttt{DoubleX4\#}
values) may or may not be relaxed with wider registers---empirical
testing is required.

%% ------------------------------------------------------------------
\subsection{Summary of Remaining Opportunities}

\begin{center}
\footnotesize
\begin{tabular}{cl@{\;\;}c@{\;\;}c@{\;\;}c@{\;\;}c}
\toprule
\textbf{\#} & \textbf{Optimisation} & \textbf{Status} & \textbf{Measured gain} & \textbf{Primary beneficiary} & \textbf{Blocker} \\
\midrule
\multicolumn{6}{l}{\textit{Implemented in Round~20}} \\
1 & GEMM micro-panel packing      & Done (20g)  & $0.092 \to 0.084\times$   & GEMM ($n \geq 256$)           & --- \\
2 & 2D parallel GEMM tiling       & Done (20h)  & Parallel scaling           & Parallel GEMM ($\geq 4$ thr.) & --- \\
\midrule
\multicolumn{6}{l}{\textit{Implemented in Rounds~21--22}} \\
8 & D\&C bidiag SVD (\texttt{svdGKP})  & Done (21a)  & ${\sim}20\%$ svdGKP & SVD (all sizes)   & --- \\
9 & D\&C eigen dispatch (\texttt{svdAtAP}) & Done (21b)  & SVD $1.59 \to 1.37\times$ & SVD ($n \geq 50$) & --- \\
10 & BLAS-3 panel bidiag           & Done (22a)  & SVD 200 $0.89 \to 0.73\times$ & SVD ($n \geq 128$) & --- \\
11 & SVD pipeline tuning           & Done (22b--d) & Incremental               & SVD (all sizes)   & --- \\
\midrule
\multicolumn{6}{l}{\textit{Implemented in Round~23}} \\
12 & Parameter tuning (5 items)    & Done (23a)  & SVD $1.38 \to 1.27\times$ & SVD ($n \leq 100$) & --- \\
13 & SIMD \texttt{sumZSq}          & Done (23b)  & Incremental               & D\&C eigensolver  & --- \\
14 & SIMD secular eval (3 funcs)   & Reverted    & NaN failures              & ---               & GHC SIMD \\
\midrule
\multicolumn{6}{l}{\textit{Remaining --- deferred for low impact at benchmarked sizes}} \\
3 & Cache/panel auto-tuning       & Ready   & $1.05\text{--}1.20\times$ & All (other hardware)           & None \\
\midrule
\multicolumn{6}{l}{\textit{Remaining --- requires AVX-512 hardware and LLVM backend testing}} \\
4 & AVX-512 / \texttt{DoubleX8\#} & Ready$^*$ & $1.5\text{--}1.8\times$   & All SIMD kernels              & See \S\ref{sec:future-avx512} \\
\midrule
\multicolumn{6}{l}{\textit{Remaining --- requires GHC SIMD enhancement}} \\
5 & Per-lane conditional SIMD      & Blocked  & ${\sim}2\times$ secular   & D\&C eigensolver  & GHC primops \\
\bottomrule
\end{tabular}
\end{center}

Items~1--2 were implemented in Round~20 Sub-rounds 20g--20h.
Items~8--11 in Rounds~21--22.  Items~12--14 in Round~23.  Item~14
(SIMD secular evaluation) was implemented and reverted: the inner loops
divide by $d_i - \mu$, which can be near-zero, and GHC lacks per-lane
conditional SIMD (\texttt{blendvpd}) to skip these entries.

Of the remaining items: Item~3 (cache auto-tuning) has no external
blockers but is deferred because the development machine's cache
hierarchy is the only one benchmarked.  Item~4 (AVX-512) is technically
feasible today via the LLVM backend with \texttt{-mavx512f} (the
\texttt{DoubleX8\#} primops already exist in GHC), but requires
AVX-512 hardware and empirical testing
(Section~\ref{sec:future-avx512}).  Item~5 (per-lane conditional SIMD)
requires a GHC primop enhancement to expose masked vector operations.

Four empirically eliminated targets remain excluded:
\begin{itemize}
\item \textbf{GEMM $k$-loop unrolling} --- \textit{blocked by GHC
  register allocator.}  The LLVM~17 backend spills to the stack beyond
  ${\sim}14$ simultaneous \texttt{DoubleX4\#} values, causing a
  $20\text{--}30\%$ regression.  This is a GHC/LLVM code generator
  limitation, not an algorithmic one.
\item \textbf{Raw \texttt{svdGKP} as drop-in default} ---
  \textit{partially addressed in Rounds~21--23.}  The D\&C bidiagonal SVD
  (Round~21a), BLAS-3 panel bidiagonalisation (Round~22a), and
  parameter tuning (Round~23a) improved \texttt{svdGKP}, but it
  remains $2\text{--}3\times$ slower than \texttt{svdAtAP} due to
  higher constant factors in the bidiagonalisation + D\&C merge
  pipeline.
\item \textbf{D\&C eigensolver crossover below $n = 1000$} ---
  \textit{addressed in Round~21.}  The D\&C eigensolver is now used
  directly in \texttt{svdAtAP} for $n \geq 50$ (Round~21b), where it
  proved dramatically faster than QR iteration for eigenvector
  accumulation.  The standalone \texttt{symmetricEigenPDC} retains
  $\texttt{dcCrossover} = 1000$ for the eigenSH benchmarks, but the
  SVD path benefits from D\&C at all sizes $\geq 50$ because the
  eigendecomposition of $A^\top A$ has particularly favourable GEMM
  ratios.
\item \textbf{SIMD secular evaluation} --- \textit{attempted and
  reverted in Round~23b.}  The secular function $f(\mu) = 1 + \rho
  \sum z_i^2 / (d_i - \mu)$ requires per-element conditional skipping
  of near-pole entries ($|d_i - \mu| < 10^{-300}$).  Without GHC
  per-lane conditional SIMD primitives (\texttt{blendvpd}), vectorised
  division produces Inf/NaN that contaminates the accumulator.  Only
  \texttt{sumZSq} (no division) was successfully vectorised.
\end{itemize}

The remaining SVD deficit ($1.27\times$ at $100{\times}100$) reflects
a fundamental tradeoff between performance and numerical quality
(Section~\ref{sec:svd-tradeoff}).  The fastest path (\texttt{svdAtAP})
uses the numerically inferior $A^\top A$ eigendecomposition, which
squares the condition number.  LAPACK achieves both speed and numerical
soundness because its Golub--Kahan bidiagonalisation path
(\texttt{DGEBRD} + \texttt{DBDSDC}) has been micro-optimised over
decades with Fortran BLAS-3 routines.  \texttt{linear-massiv}'s
numerically proper path (\texttt{svdGKP}) implements the same algorithm
but has not yet reached LAPACK's level of bidiagonalisation
micro-optimisation, remaining $2\text{--}3\times$ slower than
\texttt{svdAtAP}.

To understand the remaining gap, consider the approximate FLOP
breakdown for each path at $n = m = 100$:

\begin{center}
\small
\begin{tabular}{lrrr}
\toprule
\textbf{Phase} & \textbf{svdAtAP} & \textbf{svdGKP} & \textbf{LAPACK \texttt{dgesdd}} \\
\midrule
$A^\top A$ formation          & 2\,M   & ---     & ---     \\
Bidiagonalisation             & ---     & 27\,M  & 13\,M  \\
$U_0$/$V_0$ accumulation      & ---     & 7\,M   & 4\,M   \\
Tridiagonalisation            & 4\,M   & ---     & ---     \\
Eigensolve / bidiag SVD       & 15\,M  & 25\,M  & 12\,M  \\
$U$-recovery GEMM             & 20\,M  & ---     & ---     \\
Sort + sign fix               & 0.2\,M & 0.1\,M & 0.1\,M \\
\midrule
\textbf{Total}                & \textbf{41\,M} & \textbf{59\,M} & \textbf{29\,M} \\
\bottomrule
\end{tabular}
\end{center}

LAPACK's advantage over \texttt{svdGKP} comes from two sources:
(i)~BLAS-3 panel bidiagonalisation (\texttt{DGEBRD}) with $n_b = 64$
at $n = 100$---the panel path fires because LAPACK has no crossover
guard---halving the bidiagonalisation cost; and (ii)~\texttt{DBDSDC}'s
highly-tuned Fortran D\&C merge step with deflation bookkeeping that
minimises secular equation evaluations.

\texttt{svdAtAP}'s advantage over \texttt{svdGKP} comes from bypassing
bidiagonalisation entirely: forming $A^\top A$ costs only $2n^3$ FLOPs
(vs.\ $\tfrac{8}{3}n^3$ for Level-2 bidiagonalisation), and the
tridiagonalisation of the resulting $n{\times}n$ symmetric matrix costs
only $\tfrac{4}{3}n^3$ FLOPs.  However, the $20$\,M-FLOP $U$-recovery
GEMM ($U = A \cdot V_{\text{scaled}}$) is the single most expensive
phase in \texttt{svdAtAP} at $n = 100$, and there is no obvious way to
reduce it below $2n^3$.

Closing the remaining gap has two avenues:

\paragraph{Avenue~(a): make \texttt{svdGKP} competitive with LAPACK's
\texttt{dgesdd}.}

Rounds~21a, 22a, and 23a implemented the major algorithmic building
blocks and parameter tuning: D\&C bidiagonal SVD, BLAS-3 panel
bidiagonalisation, panel bidiag crossover lowered to 64, WY block
size raised to 48, D\&C base case raised to 32.  Round~23b attempted
SIMD secular evaluation but only \texttt{sumZSq} could be vectorised
(the others were reverted due to near-pole NaN).  The remaining
constant-factor gaps between \texttt{svdGKP} and LAPACK are:

\begin{enumerate}
\item \textbf{Bidiagonalisation panel size at $n = 100$ is small.}
  With the crossover now at $n = 64$ (lowered in Round~23a), panel
  bidiagonalisation fires at $n = 100$.  However, the panel size
  ($n_b = \min(32, \max(8, 100/6)) = 16$) is small: early panels'
  trailing GEMMs operate on near-full-size submatrices and amortise
  well, but later panels' GEMMs are small.  Increasing the panel
  size formula to $n/4$ or $n/3$ (matching what was done for
  tridiagonalisation) would produce $n_b = 25\text{--}33$ at
  $n = 100$, giving larger trailing GEMMs.

  \textit{Risk:} Larger panels increase the Level-2 work within each
  panel (GEMV operations to maintain X/Y accumulators), which may
  offset the Level-3 trailing GEMM benefit at $n = 100$.

\item \textbf{D\&C bidiagonal merge step overhead.}
  At each of the ${\sim}\log_2(100) \approx 7$ merge levels, the D\&C
  bidiagonal SVD performs insertion sort, secular equation solve,
  Gu--Eisenstat eigenvector construction, and two GEMMs.  The GEMMs
  are fast but the secular and eigenvector phases are
  scalar-dominated.  Round~23b vectorised \texttt{sumZSq} but
  the core secular function (\texttt{secularFuncAndDeriv}) and pole
  sums (\texttt{farPoleSum}, \texttt{farPoleSumSkip}) remain scalar
  due to the near-pole conditional skip that GHC SIMD cannot express.

  Specific remaining opportunities:
  \begin{itemize}
  \item \textbf{Per-lane conditional SIMD (GHC enhancement).}
    If GHC exposed a \texttt{blendDoubleX4\#} or masked-divide
    primop, the secular functions could be vectorised without
    pre-processing.  This is the single largest remaining SIMD
    opportunity for the D\&C eigensolver.
  \item \textbf{Batched deflation.}  Close-$d$ Givens deflation
    currently processes one pair at a time.  Batching into a blocked
    WY update would convert scalar Givens into GEMM calls.
  \item \textbf{Merge-level pruning.}  A fast-path for the
    no-deflation case could save ${\sim}5\%$ at large $n$.
  \end{itemize}
\end{enumerate}

\textit{Overall assessment.}  After Round~23's parameter tuning,
\texttt{svdGKP} has exhausted the easy constant-factor wins.  The
remaining gap to LAPACK's \texttt{dgesdd} is in the D\&C merge step
overhead: LAPACK's Fortran D\&C has decades of micro-optimisation
in deflation bookkeeping and secular equation evaluation that
\texttt{linear-massiv}'s Haskell implementation has not matched.
Further improvement requires either GHC SIMD enhancements (per-lane
conditionals) or algorithmic changes to the merge step (batched
deflation, merge-level pruning).  This is the path that preserves
full numerical quality ($\kappa$ preserved throughout).

\paragraph{Avenue~(b): reduce the constant factor of \texttt{svdAtAP}.}

Rounds~22b--23a implemented all proposed parameter changes.  The
updated FLOP breakdown for \texttt{svdAtAP} at $n = m = 100$ is:

\begin{center}
\small
\begin{tabular}{lrl}
\toprule
\textbf{Phase} & \textbf{Cost} & \textbf{Status} \\
\midrule
$A^\top A$ (SIMD GEMM)       & 2\,M   & Near-optimal (fused DSYRK) \\
Tridiagonalisation (panel)    & 4\,M   & Panel $n_b = 33$ at $n = 100$ (R23a: $n/3$) \\
D\&C eigensolve               & 15\,M  & D\&C via \texttt{symmetricEigenPDC} ($n \geq 50$) \\
$U$-recovery (pre-scaled GEMM)& 20\,M  & Single GEMM, packed $B$ (R23a: crossover 96) \\
Sort + permute                & 0.2\,M & O(1)-indexed permutation \\
\midrule
\textbf{Total}                & \textbf{41\,M} & \\
\bottomrule
\end{tabular}
\end{center}

Round~23a confirmed that \texttt{svdAtAP} already calls
\texttt{symmetricEigenPDC} directly for $n \geq 50$, which
unconditionally uses the D\&C tridiagonal eigensolver---the
\texttt{dcCrossover = 1000} guard only applies to the standalone
\texttt{symmetricEigenP} entry point.  The GEMM packing crossover
was lowered from 128 to 96, enabling packed-$B$ GEMM for the
$U$-recovery GEMM.  The tridiag panel size was changed from $n/4$
to $n/3$, giving $n_b = 33$ at $n = 100$ (3~panels instead of 4).

Remaining opportunities are limited:

\begin{enumerate}
\item \textbf{$A$-panel packing in GEMM.}
  The packed-$B$ variant is now active for the $U$-recovery GEMM
  ($\min(m,k,n) = 100 \geq 96$).  $A$-panel packing
  (column-contiguous broadcast) is not yet implemented; adding it
  would further reduce L2 misses for the $A$ reads during the
  micro-kernel's $k$-loop.

  \textit{Expected gain:} ${\sim}3\text{--}5\%$ for the $U$-recovery
  GEMM (${\sim}1\text{--}2\%$ of total SVD time).

\item \textbf{Further tridiag panel size tuning.}
  The $n/3$ formula gives $n_b = 33$ at $n = 100$.  LAPACK's
  \texttt{DSYTRD} queries ILAENV for an optimal $n_b = 64$ at
  all sizes.  Raising the formula to $n/2$ would give $n_b = 50$
  at $n = 100$, but this increases Level-2 work within each panel.
  The optimal balance depends on the ratio of GEMV to GEMM
  throughput for the specific micro-kernel.

  \textit{Observed effect:} The $n/3$ change slightly regressed
  eigenSH $200{\times}200$ ($0.63\times \to 0.77\times$) while
  improving eigenSH $100{\times}100$ ($0.96\times \to 0.94\times$),
  suggesting the optimal panel size is size-dependent.  A
  size-adaptive formula or lookup table could capture this.
\end{enumerate}

\textit{Overall assessment.}  After Round~23's parameter tuning,
\texttt{svdAtAP} has limited remaining headroom.  The $U$-recovery
GEMM ($20$\,M~FLOPs) is a single GEMM call that already uses the
packed-$B$ variant---there is no algorithmic way to reduce it below
$2n^3$.  The D\&C eigensolve ($15$\,M) already uses the fastest
available path.  Further improvements require either hardware-level
changes (AVX-512) or GEMM micro-kernel enhancements that are
currently blocked by GHC register pressure.  The $1.27\times$
deficit at $100{\times}100$ reflects the genuine FLOP overhead of the
$A^\top A$ approach ($41$\,M) compared to LAPACK's \texttt{dgesdd}
($29$\,M): LAPACK's advantage comes from lower total FLOPs via
optimised bidiagonalisation, not from faster per-FLOP throughput.

\paragraph{Avenue~(c): hybrid dispatch.}

A third avenue combines the strengths of both paths: select
\texttt{svdGKP} for ill-conditioned matrices and \texttt{svdAtAP}
for well-conditioned ones.  A cheap condition-number estimate
can be obtained from the diagonal of $R$ in a preliminary thin QR
factorisation of $A$ (cost: $\tfrac{2}{3}n^3$ FLOPs for $m = n$,
or ${\sim}30\%$ overhead):
\[
  \hat{\kappa}(A) = \frac{\max_i |R_{ii}|}{\min_i |R_{ii}|}
\]
If $\hat{\kappa} > 10^6$, dispatch to \texttt{svdGKP}; otherwise
use \texttt{svdAtAP}.  This ensures numerical robustness where it
matters while maintaining peak throughput for the common case of
well-conditioned matrices.

Alternatively, the condition-number estimate could be extracted
from the bidiagonalisation itself (the ratio of the largest to
smallest diagonal entry of $B$ approximates $\kappa(A)$), which
would allow a ``try \texttt{svdAtAP}, verify, fall back'' strategy
with no upfront overhead for well-conditioned inputs.

\paragraph{Not recommended at current benchmark sizes.}
Strassen/Winograd multiplication (not blocked, but breakeven at
$n > 2000$; benchmarks cap at $n = 500$), GPU offloading (blocked by
design---breaks the pure-Haskell, zero-FFI premise), cache-oblivious
algorithms (blocked by GHC---recursion overhead from thunk allocation
and garbage collection negates the theoretical cache benefit), and
mixed-precision iterative refinement (not blocked, but unnecessary---
${\sim}10^{-12}$ tolerance is already achieved by direct methods) are
...
categories of numerical operations through twenty-three rounds of optimisation.
...
to $7.6\text{--}33\times$ faster} than hmatrix/LAPACK (up to $115\times$
internal speedup).
...
$41\text{--}149\times$ slower to exact parity with hmatrix/LAPACK}
...
$1.13\times$ loss to a \textbf{$1.76\times$ victory} over hmatrix/LAPACK
...
to $\mathbf{0.50\times}$ ($2\times$ faster than hmatrix/LAPACK).  A
...
vs.\ hmatrix/LAPACK---near parity.  SVD improved to $1.64\text{--}2.08\times$.
...
faster than hmatrix/LAPACK} at $500{\times}500$ ($0.51\times$);
\textbf{SVD faster than hmatrix/LAPACK} at $500{\times}500$ ($0.90\times$);
...
faster than hmatrix/LAPACK} ($0.48\times$, improved from $0.63\times$);
SVD $500{\times}500$ improved to $0.76\times$ (from $0.90\times$);
eigenSH $500{\times}500$ tightened to $0.47\times$ (from $0.51\times$).

\paragraph{Round~19 (eigenvalue/SVD convergence).}
Row-major QR dispatch, adaptive panel thresholds, stall detection, and
aggressive early deflation brought eigenSH to parity at $100{\times}100$
($1.00\times$) and $1.4\times$ faster at $200{\times}200$ ($0.69\times$).
SVD $200{\times}200$ reached parity ($0.98\times$, improved from
$1.53\times$).  These gains came primarily from reducing the iteration
count in QR eigenvalue convergence rather than from faster per-iteration
work.

\paragraph{Round~20 (algorithmic deepening).}
Eight sub-rounds: fused DSYRK kernel, column-major SIMD Givens for
bidiagonal QR, blocked bidiagonalisation with WY accumulation, D\&C
eigensolver stabilisation via Gu--Eisenstat perturbation-based
deflation, panel LU and Cholesky factorisation, double-shift QR, GEMM
micro-panel packing, and 2D parallel GEMM tiling.  \textbf{LU solve
$2.1\times$ faster} than hmatrix/LAPACK ($0.47\times$, from
$0.68\times$); \textbf{GEMM $11.9\times$ faster} at $500{\times}500$
($0.084\times$) via packing; SVD deficit reduced to $1.59\times$ at
$100{\times}100$; eigenSH passed parity at $100{\times}100$
($0.96\times$) and reached $0.49\times$ at $500{\times}500$.

\paragraph{Round~21 (D\&C SVD breakthrough).}
Two changes to the SVD pipeline: (1)~D\&C bidiagonal SVD for
\texttt{svdGKP}, reusing the tridiagonal D\&C's secular equation
infrastructure on squared singular values (${\sim}20\%$ improvement);
(2)~switching \texttt{svdAtAP}'s eigendecomposition from QR iteration to
the D\&C eigensolver for $n \geq 50$, which converts scalar Givens
eigenvector accumulation into $O(\log n)$ GEMM-dominated merge levels.
The dispatch change was the most impactful SVD optimisation since
Round~6: \textbf{SVD $200{\times}200$ moved from $1.17\times$ deficit to
$0.89\times$} (faster than LAPACK); \textbf{SVD $500{\times}500$ from
$0.77\times$ to $0.61\times$} ($1.6\times$ faster); SVD
$100{\times}100$ deficit reduced from $1.59\times$ to $1.37\times$.

\paragraph{Round~22 (BLAS-3 pipeline tuning).}
Four complementary SVD pipeline optimisations: DLABRD-style panel
bidiagonalisation for \texttt{svdGKP} (converting Level-2
bidiagonalisation to Level-3 for $n \geq 128$); tridiagonalisation
parameter tuning (crossover $128 \to 64$, max panel $48 \to 64$);
GEMM packing crossover lowered ($256 \to 128$); and pre-scaled V
U-recovery with DoubleX4\# SIMD for \texttt{svdAtAP}.
\textbf{SVD $200{\times}200$ from $0.89\times$ to $0.73\times$}
($37\%$ faster than LAPACK); SVD $50{\times}50$ improved from
$1.82\times$ to $1.73\times$.  SVD $100{\times}100$ and
$500{\times}500$ held steady.

\paragraph{Round~23 (parameter tuning and partial SIMD).}
Systematic implementation of all eight optimisations proposed in the
Round~22 analysis avenues~(a) and~(b): panel bidiag crossover
$128 \to 64$, WY block size $32 \to 48$, D\&C bidiag base case
$25 \to 32$, GEMM packing crossover $128 \to 96$, tridiag panel
size $n/4 \to n/3$, and SIMD \texttt{sumZSq}.  Three additional
SIMD secular functions were implemented but reverted: the near-pole
division-by-zero issue in \texttt{secularFuncAndDeriv},
\texttt{farPoleSum}, and \texttt{farPoleSumSkip} cannot be handled
without per-lane conditional SIMD, which GHC does not provide.
\textbf{SVD $100{\times}100$ deficit reduced from $1.38\times$ to
$1.27\times$}; \textbf{SVD $500{\times}500$ from $0.61\times$ to
$0.54\times$} ($1.84\times$ faster than LAPACK); eigenSH
$100{\times}100$ improved to $0.94\times$.

\paragraph{Summary.}
Of the nine benchmarked operation categories, \texttt{linear-massiv}
now \textbf{outperforms \texttt{hmatrix} (OpenBLAS/LAPACK) in all
nine at $500{\times}500$}: GEMM ($0.098\text{--}0.15\times$,
single-threaded), dot product ($0.34\times$),
matrix--vector multiply ($0.51\times$), LU solve ($0.51\times$),
Cholesky solve ($0.53\times$), QR factorisation ($0.021\times$),
eigenvalue decomposition ($0.49\text{--}0.94\times$), and SVD
($0.54\text{--}1.27\times$).  At $500{\times}500$---the largest
benchmark size---every category is faster than hmatrix/LAPACK, with
SVD $1.84\times$ faster ($0.54\times$).
...
Section~\ref{sec:future-optimisations} identifies three remaining
optimisation opportunities: cache auto-tuning, AVX-512 via
\texttt{DoubleX8\#}, and per-lane conditional SIMD (a GHC primop
enhancement).  Rounds~21--23 produced the most impactful SVD
improvements since Round~6: D\&C eigensolver dispatch (Round~21),
BLAS-3 panel bidiagonalisation (Round~22), and systematic parameter
tuning with partial SIMD (Round~23) leverage the
$6\text{--}10\times$ faster GEMM kernel, converting scalar Givens
sweeps and Level-2 Householder updates into GEMM-dominated
computation.  SVD $100{\times}100$ deficit reduced from $1.59\times$
(Round~20) to $1.27\times$ (Round~23); SVD $500{\times}500$ from
$0.77\times$ to $0.54\times$ ($1.84\times$ faster than LAPACK).
