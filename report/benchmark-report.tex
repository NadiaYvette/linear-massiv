...
The trajectory reveals five distinct phases:
\begin{enumerate}
...
\item[\texttt{linear-massiv}] Now the performance leader in \textbf{all
  ...
  $1.1\times$ faster at $200{\times}200$ and $1.6\times$ faster at
  $500{\times}500$; SVD is $1.4\times$ faster at $200{\times}200$;
  LU solve is $2.1\times$ faster at $100{\times}100$.
...
\toprule
\textbf{\#} & \textbf{Optimisation} & \textbf{Status} & \textbf{Expected gain} & \textbf{Primary beneficiary} & \textbf{Blocker} \\
\midrule
...
\item[(a)] \textbf{Make \texttt{svdGKP} competitive with LAPACK's
  \texttt{dgesdd}:} This requires a faster bidiagonalisation
  (BLAS-3 panel bidiagonalisation analogous to \texttt{DGEBRD},
  which accumulates Householder reflectors in blocks rather than
  one-at-a-time) and further optimisation of the D\&C bidiagonal SVD
  merge step.  This is the path that preserves numerical quality.
\item[(b)] \textbf{Reduce the constant factor of \texttt{svdAtAP}:}
  E.g.\ via more efficient tridiagonalisation, GEMM packing at
  smaller sizes, or a cheaper $U$-recovery step.  This improves
  benchmark numbers but does not address the condition-number
  squaring limitation.
\end{enumerate}
