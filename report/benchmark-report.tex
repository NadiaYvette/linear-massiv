...
After five rounds of optimisation---algorithmic improvements (cache-blocked
...
higher-level algorithms (LU, Cholesky, QR, eigenvalue),
P-specialised SVD pipeline wiring, and parallel
GEMM---\texttt{linear-massiv} now \textbf{outperforms hmatrix
(OpenBLAS) in seven of nine benchmarked operation categories}:
GEMM is $2\times$ faster at $200 \times 200$ single-threaded and
\textbf{$14\times$ faster at $500 \times 500$ with 20~cores};
dot product is $3\text{--}11\times$ faster; LU solve is
$1.8\text{--}2.8\times$ faster; Cholesky solve is
$1.1\text{--}3.1\times$ faster; and QR factorisation is
$7.5\text{--}33\times$ faster.  SVD improved $3\times$ through
pipeline optimisation but remains $22\text{--}94\times$ slower.
...
categories of numerical operations through five rounds of optimisation.
...
\paragraph{Summary.}
Of the nine benchmarked operation categories, \texttt{linear-massiv}
now \textbf{outperforms \texttt{hmatrix} (OpenBLAS/LAPACK) in seven}:
GEMM (single-threaded and parallel), dot product, matrix--vector
multiply, LU solve, Cholesky solve, and QR factorisation.  Parallel
GEMM is the standout result, achieving a \textbf{$14\times$ advantage
over OpenBLAS} at $500 \times 500$.  The remaining gaps in eigenvalue
($41\times$ at 10$\times$10, $149\times$ at 50$\times$50) and SVD
($22\times$ at 10$\times$10, $94\times$ at 50$\times$50) are
confined to a single algorithmic bottleneck: the Householder
tridiagonalisation, which still uses Haskell lists and massiv's
per-element abstraction.
...
FFI-based BLAS/LAPACK} across the majority of numerical linear
