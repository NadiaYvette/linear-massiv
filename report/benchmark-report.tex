...
Section~\ref{sec:remedies} proposes six concrete optimisation
directions for closing the gap, ordered by expected impact: (1)~in-place
factorisation via the ST monad, eliminating the per-iteration matrix
allocation that dominates QR, eigenvalue, and SVD runtimes;
(2)~implicit Householder (compact WY) representation to avoid forming
explicit $Q$ matrices; (3)~cache-blocked tiled GEMM and loop
reordering for better cache utilisation; (4)~divide-and-conquer
eigenvalue and SVD algorithms; (5)~SIMD primitives via GHC's
\texttt{DoubleX4\#} for inner-loop vectorisation; and (6)~an optional
FFI backend delegating to system BLAS/LAPACK for users who can accept
the dependency.  These optimisations are cumulative: the first three
alone could plausibly reduce the GEMM gap from $329\times$ to
$\sim 30\text{--}50\times$, and the in-place refactoring should
dramatically improve factorisation and iterative solver performance.
