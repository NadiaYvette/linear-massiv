...
\midrule
$10{\times}10$   & 3.58  & 1.33  & 1.46  & 0.37  & 0.41 \\
$50{\times}50$   & 40.7  & 30.9  & 22.6  & 0.76  & 0.56 \\
$100{\times}100$ & 224   & 128   & 115   & 0.57  & 0.51 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{SVD: Round~6 vs.\ Round~7 ($+$RTS $-$N1, \texttt{svdAtAP})}
\label{tab:round7-svd}
\begin{tabular}{l S[round-precision=1] S[round-precision=1] S[round-precision=1] S[round-precision=1] S[round-precision=1]}
\toprule
{Size} & {hmatrix (\si{\micro\second})} & {lm R6 (\si{\micro\second})} & {lm R7 (\si{\micro\second})} & {R6 ratio} & {R7 ratio} \\
\midrule
$10{\times}10$   & 27.7  & 60.8  & 85.0  & 2.2 & 3.1 \\
$50{\times}50$   & 522   & 2429  & 2036  & 4.7 & 3.9 \\
$100{\times}100$ & 2899  & 9375  & 10230 & 3.2 & 3.5 \\
\bottomrule
\end{tabular}
\end{table}
...
The SIMD column kernel delivers a \textbf{$1.8\text{--}1.9\times$ speedup
over hmatrix} at $50{\times}50$ and $100{\times}100$, flipping the
$100{\times}100$ case from a $1.3\times$ loss in Round~6 to a
$1.95\times$ win.  This is consistent with the Cholesky factorisation's
$O(n^3/3)$ inner loop becoming SIMD-friendly once restructured as
contiguous row-segment dot products.  At $10{\times}10$, the slight
regression (from $0.37\times$ to $0.41\times$) is within noise and
still represents a $2.5\times$ advantage over hmatrix.
...
than LAPACK.  The Golub--Kahan implementation is retained as
\texttt{svdGKP} for applications where numerical conditioning matters
...
Dot product & 1000 & $0.35\times$ & \texttt{linear-massiv} \\
Matrix--vector & 100 & $0.49\times$ & \texttt{linear-massiv} \\
LU solve & $100{\times}100$ & $0.75\times$ & \texttt{linear-massiv} \\
Cholesky solve & $100{\times}100$ & $0.51\times$ & \texttt{linear-massiv} \\
QR factorisation & $100{\times}100$ & $0.031\times$ & \texttt{linear-massiv} \\
Eigenvalue (eigenSH) & $50{\times}50$ & $0.99\times$ & parity \\
SVD & $50{\times}50$ & $3.9\times$ & hmatrix \\
\bottomrule
\end{tabular}
\end{table}
...
previous $100{\times}100$ loss into a decisive $1.95\times$ victory.
...
\item \textbf{Divide-and-conquer tridiagonal eigensolver.}
  The eigenvalue ratio at $100{\times}100$ ($1.6\times$) reflects the
...
$1.3\times$ loss to a \textbf{$1.95\times$ victory} over LAPACK.
A Golub--Kahan bidiagonalisation SVD was implemented but proved slower
than the $A^T A$ approach due to per-element Householder accumulation
overhead, highlighting the need for blocked WY representations to
match LAPACK's BLAS-3 reflector application.
...
decomposition.  The sole remaining disadvantage is SVD
($3\text{--}4\times$), which requires blocked Householder
representations or a divide-and-conquer approach to close.
