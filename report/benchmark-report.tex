...
\midrule
\multicolumn{6}{l}{\textit{Medium priority --- scaling beyond benchmarked sizes}} \\
1 & GEMM micro-panel packing       & Medium & Medium & $1.1\text{--}1.3\times$   & GEMM ($n > 1000$) \\
\midrule
\multicolumn{6}{l}{\textit{Lower priority --- infrastructure and scaling}} \\
2 & 2D parallel GEMM tiling        & Low    & Medium & $1.2\text{--}1.4\times$   & Parallel GEMM ($16{+}$ threads) \\
3 & Cache/panel auto-tuning        & Low    & Low    & $1.05\text{--}1.20\times$ & All (non-default hardware) \\
4 & AVX-512 / \texttt{DoubleX8\#}  & Low    & Medium & $1.5\text{--}1.8\times$   & All SIMD kernels \\
\bottomrule
\end{tabular}
\end{center}
...
deferred due to insufficient benefit relative to complexity.  Three
empirically eliminated targets remain excluded: GEMM $k$-loop unrolling
(GHC's LLVM~17 backend spills registers beyond ${\sim}14$ simultaneous
\texttt{DoubleX4\#} values), raw \texttt{svdGKP} as drop-in replacement
(now superseded by the blocked bidiagonalisation path), and D\&C
crossover tuning below $n = 1000$ (merge-step GEMM overhead does not
amortise at smaller sizes).
...
\texttt{svdGKP} competitive with LAPACK's DBDSQR (currently limited by
the scalar Givens chase), or (b)~lowering the D\&C eigensolver
crossover below $n = 1000$ once the merge-step GEMM overhead is reduced.
