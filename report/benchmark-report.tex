...
%% ====================================================================
\section{Round~20: Fused DSYRK, Blocked SVD, D\&C Stabilisation, and Panel Solvers}
\label{sec:round20}

Round~20 implements six of the nine optimisations from the
Chapter~\ref{sec:future-optimisations} roadmap in five sub-rounds
(20a--20e), targeting SVD, eigenSH, LU, and Cholesky performance at
all sizes.  Three infrastructure optimisations (GEMM packing, 2D
parallel tiling, cache auto-tuning) are deferred as they provide
negligible benefit at $n \leq 500$.
...
\textbf{Multi-shift QR (Opt~4):} Double-shift bulge chase for
symmetric tridiagonal matrices.  Deferred because the Wilkinson single
shift already provides cubic convergence; the expected
$1.10\text{--}1.15\times$ improvement was not justified by the
implementation complexity.

\textbf{GEMM packing (Opt~6), 2D parallel GEMM (Opt~7), cache
auto-tuning (Opt~8):} All provide negligible benefit at $n \leq 500$
(the benchmarked range).  GEMM packing benefits only at $n \geq 512$;
2D tiling only affects parallel benchmarks; cache auto-tuning provides
at most $1.05\text{--}1.20\times$ on non-default hardware.

\textbf{AVX-512 (Opt~9):} GHC~9.14 does not expose
\texttt{DoubleX8\#} primops.  Pending GHC support.
...
GEMM $500{\times}500$       & 220\,ms   & 20.3\,ms       & $0.092\times$ & $=$ \\
...
\textbf{LU solve:} Panel factorisation reduces the ratio at
$100{\times}100$ from $0.68\times$ to $0.47\times$, a 31\% improvement.
The Level-3 GEMM trailing update converts the cache-unfriendly
column-by-column elimination into a single cache-friendly tiled
operation.  The benefit will be larger at $n \geq 200$ where the
GEMM update dominates.

\textbf{D\&C eigensolver stability:} The perturbation-based deflation
criterion eliminates the catastrophic orthogonality loss that disabled
the D\&C path.  When $\rho z_i^2 < \varepsilon |d_i|$, the secular
solver returned $\lambda_i = d_i$ exactly, causing the Gu--Eisenstat
eigenvector formula to produce $z_i^{(\text{new})} = 0$---zeroing the
dominant component.  The fix deflates these entries proactively,
achieving reconstruction errors $< 5 \times 10^{-7}$ at $n = 128$.
The D\&C path is now stable but remains slower than QR at $n \leq 500$
($\texttt{dcCrossover} = 1000$) due to merge-step GEMM overhead.

\textbf{SVD $100{\times}100$:} The fused DSYRK kernel, column-major
SIMD Givens in bidiagonal QR, and blocked bidiagonalisation combine to
reduce the SVD deficit from $1.86\times$ to $1.59\times$.  The
remaining gap is primarily due to the $A^\top A$ eigendecomposition
overhead (two extra GEMMs); the direct Golub--Kahan path
(\texttt{svdGKP}) with blocked bidiagonalisation remains
${\sim}2\times$ slower at $100{\times}100$ due to the scalar
bidiagonal QR phase.
...
and eigenSH at $n = 10$ (baseline overhead).
...
GEMM $500{\times}500$       & 220\,ms   & 20.3\,ms       & $0.092\times$ & $10.8\times$ \\
...
eigenSH $500{\times}500$    & 240\,ms   & 129\,ms        & $0.54\times$ & $1.9\times$ \\
SVD $100{\times}100$        & 3.74\,ms  & 5.96\,ms       & $1.59\times$ & --- \\
SVD $200{\times}200$        & 25.1\,ms  & 29.4\,ms       & $1.17\times$ & --- \\
SVD $500{\times}500$        & 416\,ms   & 336\,ms        & $0.81\times$ & $1.2\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
operation categories}.  After Round~20, eigenSH is $1.04\times$ faster
at $100{\times}100$ and $1.6\times$ faster at $200{\times}200$; LU solve
has improved from $1.5\times$ to $2.1\times$ faster via panel factorisation.
The remaining deficits are SVD at $100{\times}100$ ($1.59\times$) and
$200{\times}200$ ($1.17\times$), inherent to the $A^\top A$
eigendecomposition approach despite fused DSYRK, blocked
bidiagonalisation, and SIMD Givens optimisations applied in Round~20.
...
eigenSH $500{\times}500$  & ---            & ---          & ---          & ---          & $0.51\times$ & $0.47\times$ & $0.48\times$ & $0.54\times$ \\
SVD $500{\times}500$      & ---            & ---          & ---          & ---          & $0.90\times$ & $0.76\times$ & $0.82\times$ & $0.81\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{Round~20 (algorithmic deepening):} Fused DSYRK, blocked
  bidiagonalisation, D\&C eigensolver stabilisation, and panel
  LU/Cholesky factorisation.  LU improved from $0.68\times$ to
  $0.47\times$ ($2.1\times$ faster than hmatrix/LAPACK); SVD deficit
  reduced from $1.86\times$ to $1.59\times$ at~100; eigenSH passed
  parity at~100 ($0.96\times$).
\end{enumerate}
...
implemented six of the nine optimisations proposed after Round~19:
fused DSYRK (item~3), blocked Golub--Kahan SVD with SIMD Givens
(item~1), D\&C eigensolver stabilisation (item~2), panel LU/Cholesky
(item~5), while deferring multi-shift QR (item~4, insufficient
benefit vs.\ complexity) and three infrastructure items (6--8).
...
Items~1--3 and~5 from the previous roadmap are now implemented
(Section~\ref{sec:round20}).  Item~4 (multi-shift QR) was evaluated
and deferred (Wilkinson single-shift already provides cubic
convergence; expected gain $1.10\text{--}1.15\times$ did not justify
complexity).  Remaining opportunities focus on infrastructure scaling
and future GHC capabilities.
...
\textbf{Deferred:} Multi-shift QR (double-shift bulge chase) was
evaluated and deferred because the Wilkinson single-shift already
provides cubic convergence for symmetric tridiagonal matrices, and the
expected $1.10\text{--}1.15\times$ improvement does not justify the
implementation complexity.
...
because the benefit is negligible at $n \leq 500$.
...
\paragraph{Status: implementable now.}  No external blockers; deferred
because single-threaded benchmarks are the primary comparison target
and this only affects parallel performance.
...
\paragraph{Status: blocked by GHC.}  GHC~9.14 does not expose
\texttt{DoubleX8\#} primops.  The LLVM~17 backend can generate
AVX-512 instructions when passed \texttt{-mavx512f}, but without
corresponding Haskell-level primops there is no way to express 8-wide
SIMD operations in source code.  This is a hard dependency on a future
GHC version adding 512-bit SIMD primitive types.
...
\midrule
\multicolumn{6}{l}{\textit{Implementable now --- deferred for low impact at benchmarked sizes}} \\
1 & GEMM micro-panel packing      & Ready   & $1.1\text{--}1.3\times$   & GEMM ($n > 1000$)             & None \\
2 & 2D parallel GEMM tiling       & Ready   & $1.2\text{--}1.4\times$   & Parallel GEMM ($16{+}$ thr.)  & None \\
3 & Cache/panel auto-tuning       & Ready   & $1.05\text{--}1.20\times$ & All (other hardware)           & None \\
\midrule
\multicolumn{6}{l}{\textit{Blocked --- awaiting external support}} \\
4 & AVX-512 / \texttt{DoubleX8\#} & Blocked & $1.5\text{--}1.8\times$   & All SIMD kernels               & GHC primops \\
\bottomrule
\end{tabular}
\end{center}

Items~1--3 have no external blockers and could be implemented at any
time; they are deferred because their benefit is negligible at the
benchmarked sizes ($n \leq 500$, single-threaded, AMD Zen~4 cache
hierarchy).  Item~4 is blocked by a hard GHC dependency:
\texttt{DoubleX8\#} primops do not exist in GHC~9.14, and there is no
source-level workaround to express 512-bit SIMD operations.

Of the original nine proposed optimisations, six have been implemented
in Round~20 (items~1--3 and~5, plus the fused DSYRK kernel and
column-major SIMD Givens).  Multi-shift QR (item~4) was evaluated and
deferred due to insufficient benefit relative to complexity.
...
nine at $500{\times}500$}: GEMM ($0.092\text{--}0.17\times$,
...
eigenvalue decomposition ($0.54\text{--}0.96\times$), and SVD
($0.81\text{--}1.59\times$).  At $500{\times}500$---the largest
benchmark size---every category is faster than hmatrix/LAPACK, with
SVD $1.2\times$ faster ($0.81\times$).
...
Section~\ref{sec:future-optimisations} identifies four remaining
optimisation opportunities (GEMM packing, 2D parallel tiling, cache
auto-tuning, and AVX-512) that target scaling beyond the benchmarked
sizes and future GHC capabilities.
