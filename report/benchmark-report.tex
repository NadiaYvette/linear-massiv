...
Table~\ref{tab:final-results} presents the final Round~20 benchmark
...
(OpenBLAS/LAPACK), Round~20.  Ratios $< 1$ indicate \texttt{linear-massiv}
is faster.}
...
SVD $100{\times}100$        & 3.74\,ms  & 5.96\,ms       & $1.59\times$ & --- \\
SVD $200{\times}200$        & 25.1\,ms  & 29.4\,ms       & $1.17\times$ & --- \\
SVD $500{\times}500$        & 416\,ms   & 320\,ms        & $0.77\times$ & $1.3\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
operation categories}.  After Round~20, GEMM is $11.9\times$ faster at
$500{\times}500$ (up from $10.8\times$ via micro-panel packing);
eigenSH is $1.04\times$ faster at $100{\times}100$ and $2.0\times$
faster at $500{\times}500$; LU solve has improved from $1.5\times$ to
$2.1\times$ faster via panel factorisation.  The remaining deficits are
SVD at $100{\times}100$ ($1.59\times$) and $200{\times}200$
($1.17\times$), inherent to the $A^\top A$ eigendecomposition approach
despite fused DSYRK, blocked bidiagonalisation, and SIMD Givens
optimisations applied in Round~20.
...
\toprule
\textbf{Operation} & \textbf{R1} & \textbf{R3} & \textbf{R6} & \textbf{R10} & \textbf{R17} & \textbf{R18} & \textbf{R19} & \textbf{R20} \\
\midrule
GEMM $100{\times}100$    & $329\times$     & $0.50\times$ & $0.50\times$ & ---          & $0.16\times$ & $0.15\times$ & $0.15\times$ & $0.17\times$ \\
LU solve $100{\times}100$ & $295\times$    & ---          & $0.37\times$ & $0.37\times$ & $0.68\times$ & $0.68\times$ & $0.68\times$ & $\mathbf{0.47\times}$ \\
...
eigenSH $100{\times}100$  & $897\times$    & ---          & $0.93\times$ & $0.97\times$ & $1.04\times$ & $1.18\times$ & $1.00\times$ & $\mathbf{0.96\times}$ \\
eigenSH $200{\times}200$  & ---            & ---          & ---          & ---          & ---          & $1.19\times$ & $0.69\times$ & $\mathbf{0.63\times}$ \\
SVD $100{\times}100$      & $887\times$    & ---          & $3.28\times$ & $2.38\times$ & $1.80\times$ & $1.78\times$ & $1.86\times$ & $\mathbf{1.59\times}$ \\
SVD $200{\times}200$      & ---            & ---          & ---          & ---          & ---          & $1.53\times$ & $0.98\times$ & $1.17\times$ \\
eigenSH $500{\times}500$  & ---            & ---          & ---          & ---          & $0.51\times$ & $0.47\times$ & $0.48\times$ & $\mathbf{0.49\times}$ \\
SVD $500{\times}500$      & ---            & ---          & ---          & ---          & $0.90\times$ & $0.76\times$ & $0.82\times$ & $\mathbf{0.77\times}$ \\
\bottomrule
\end{tabular}
\end{table}
...
The twenty rounds of optimisation yield several cross-cutting
...
The twenty rounds of optimisation have substantially changed the
...
\item[\texttt{linear-massiv}] Now the performance leader in all nine
  ...
  is $1.04\times$ faster at $100{\times}100$ and $1.6\times$ faster at
  $200{\times}200$; LU solve is $2.1\times$ faster at $100{\times}100$.
...
After twenty rounds of optimisation, \texttt{linear-massiv}
outperforms hmatrix (OpenBLAS/LAPACK) in all nine categories at
$500{\times}500$ and eight of nine at $200{\times}200$.  Round~20
implemented seven of the nine optimisations proposed after Round~19:
fused DSYRK (item~3), blocked Golub--Kahan SVD with SIMD Givens
(item~1), D\&C eigensolver stabilisation (item~2), double-shift QR
(item~4), panel LU/Cholesky (item~5), GEMM micro-panel packing
(item~6), and 2D parallel GEMM tiling (item~7).
The remaining deficits are SVD at $n \leq 200$ ($1.59\times$ at~100,
$1.17\times$ at~200), inherent to the $A^\top A$ eigendecomposition
pathway.

The roadmap below is revised to reflect the post-Round~20 state.
Items~1--7 from the previous roadmap are now implemented
(Section~\ref{sec:round20}).  Remaining opportunities focus on
...
Of the original nine proposed optimisations, seven have been
implemented in Round~20 (Sub-rounds 20a--20h).  The two remaining
items (cache auto-tuning and AVX-512) are deferred for practical
reasons, not technical blockers.
...
\item \textbf{Raw \texttt{svdGKP} as drop-in default} ---
  \textit{blocked by bidiagonal QR performance.}  The bidiagonal QR
  Givens chase is inherently scalar and strided; even with column-major
  SIMD Givens (Round~20b), \texttt{svdGKP} remains
  $3\text{--}4\times$ slower than \texttt{svdAtAP}.  Closing this gap
  would require either a divide-and-conquer bidiagonal SVD (analogous
  to LAPACK's DBDSDC) or a fundamentally different bidiagonal QR
  iteration structure.
\item \textbf{D\&C eigensolver crossover below $n = 1000$} ---
  \textit{blocked by merge-step GEMM overhead.}  The D\&C merge step
  requires a full $n \times n$ GEMM to rotate the eigenvector matrix,
  which does not amortise against QR's tight Givens loop at
  $n \leq 500$.  Reducing the crossover requires either (a)~a more
  efficient merge-step eigenvector accumulation (avoiding the full GEMM),
  or (b)~a multi-section D\&C variant that reduces the number of merge
  levels.
\end{itemize}

The remaining SVD deficit ($1.59\times$ at $100{\times}100$) is
primarily attributable to the $A^\top A$ eigendecomposition pathway's
two extra GEMMs and condition-number squaring.  Further improvement
would require either (a)~making the bidiagonal QR phase of
\texttt{svdGKP} competitive with LAPACK's DBDSQR (blocked by the
scalar Givens chase, as noted above), or (b)~lowering the D\&C
eigensolver crossover below $n = 1000$ (blocked by merge-step overhead,
as noted above).
...
categories of numerical operations through twenty rounds of optimisation.
...
nine at $500{\times}500$}: GEMM ($0.084\text{--}0.17\times$,
...
($0.77\text{--}1.59\times$).  At $500{\times}500$---the largest
benchmark size---every category is faster than hmatrix/LAPACK, with
SVD $1.3\times$ faster ($0.77\times$).
...
scaling to other hardware and future GHC capabilities; GEMM packing
and 2D parallel tiling were implemented in Sub-rounds 20g--20h.
