...
\paragraph{Condition number squaring.}  The D\&C bidiag SVD operates
on squared singular values $d_i^2$, which squares the condition number:
...
deflation mitigate this for most practical cases.  The
\texttt{svdAtAP} path is also affected (eigendecomposition of $A^\top A$
inherently squares the condition number), so this is not a new
limitation introduced by D\&C---it is inherent to the $A^\top A$
approach.
...
The remaining SVD deficit ($1.37\times$ at $100{\times}100$) is
primarily attributable to the $A^\top A$ eigendecomposition pathway's
condition-number squaring and the cost of two GEMMs ($A^\top A$
formation and $U = A V \Sigma^{-1}$).  Further improvement would require
either (a)~making \texttt{svdGKP} competitive with \texttt{svdAtAP}
(requires a fundamentally faster bidiagonalisation), or (b)~reducing the
constant factor of the $A^\top A$ pathway (e.g.\ via a more efficient
tridiagonalisation or GEMM packing at smaller sizes).
