...
With all nine operation categories now outperforming hmatrix
(OpenBLAS/LAPACK) at $500{\times}500$, the remaining optimisation
targets focus on closing medium-size gaps (eigenSH at
$1.04\text{--}1.18\times$ for $n = 100\text{--}200$; SVD at
$1.80\times$ for $n = 200$) and deepening existing advantages.
Ten concrete targets are presented below in three priority tiers.

\subsubsection{1.\ Divide-and-Conquer Eigensolver Crossover Tuning}
\label{sec:future-dc-crossover}

\paragraph{Current state.}
A full divide-and-conquer (D\&C) tridiagonal eigensolver
(\texttt{symmetricEigenPDC}, Symmetric.hs) is implemented and tested
at small sizes ($10{\times}10$, $30{\times}30$, $50{\times}50$), with
secular equation solver, GEMM-based merge, and deflation heuristics.
However, the crossover threshold is set to $n = 100{,}000$
(Symmetric.hs:714), effectively disabling D\&C for all practical
matrix sizes.  At medium sizes ($n = 100\text{--}200$), eigenSH
ratios are $1.04\text{--}1.18\times$---the narrowest remaining gap.

\paragraph{Proposed optimisation.}
Reduce \texttt{dcCrossover} to $50\text{--}100$, benchmark D\&C
against QR at $n = 50, 100, 200, 500$, and select the crossover
that minimises wall-clock time.  D\&C replaces Level-2 QR Givens
rotations with Level-3 GEMM merges, which should favour the
register-blocked GEMM micro-kernel.

\paragraph{Expected impact.}
$1.15\text{--}1.30\times$ improvement at $n = 100\text{--}200$,
potentially pushing eigenSH below $1.0\times$ at all sizes.

%% ------------------------------------------------------------------
\subsubsection{2.\ Blocked Bidiagonalisation for SVD}
\label{sec:future-blocked-bidiag}

\paragraph{Current state.}
\texttt{svdAtAP} (SVD.hs) forms $A^\top A$ explicitly via GEMM, then
eigendecomposes the $n{\times}n$ product.  This squares the condition
number ($\kappa(A^\top A) = \kappa(A)^2$) and is ${\sim}2\times$ more
FLOP-intensive than direct bidiagonalisation.  The Golub--Kahan
implementation (\texttt{svdGKP}, SVD.hs) bidiagonalises $A$ directly
but remains $15\text{--}33\times$ slower than \texttt{svdAtAP} due to
per-element Householder accumulation.  SVD at $200{\times}200$ is
$1.80\times$---the widest remaining gap.

\paragraph{Proposed optimisation.}
Apply the same panel-based blocked WY strategy that succeeded for
eigenSH tridiagonalisation (\S\ref{sec:round16}) to the
bidiagonalisation phase of \texttt{svdGKP}:

\begin{itemize}
\item Pack $\mathtt{nb}$ left/right Householder vectors into panels,
  compute T-factors via GEMM, and apply block reflectors via Level-3
  GEMM.
\item Model on \texttt{panelTridiagP} (Symmetric.hs), which already
  implements DLATRD-style deferred updates.
\item For the bidiagonal QR iteration phase, apply the SIMD Givens
  column-major transpose strategy to both $U$ and $V$ simultaneously.
\end{itemize}

\paragraph{Expected impact.}
$1.5\text{--}2.0\times$ SVD improvement at medium sizes, potentially
bringing the $200{\times}200$ ratio from $1.80\times$ to near parity.

%% ------------------------------------------------------------------
\subsubsection{3.\ Panel-Based LU and Cholesky Factorisation}
\label{sec:future-panel-lu-chol}

\paragraph{Current state.}
Both LU (LU.hs) and Cholesky (Cholesky.hs) use column-by-column
factorisation---Level-2 BLAS operations.  Each column performs an
$O(n)$ vector update and an $O(n)$ scalar division, yielding $O(n^2)$
memory traffic per column and $O(n^3)$ total.  Current ratios are
$0.68\times$ (LU) and $0.63\times$ (Cholesky) at $100{\times}100$.

\paragraph{Proposed optimisation.}
Implement LAPACK-style panel factorisation (DGETRF/DPOTRF):
\begin{itemize}
\item Factor a panel of $\mathtt{nb}$ columns via the existing
  column-by-column kernel (Level-2).
\item Apply updates to the trailing $(n - \mathtt{nb}) \times (n - \mathtt{nb})$
  submatrix via GEMM (Level-3): TRSM for the panel solve,
  then GEMM for the rank-$\mathtt{nb}$ update.
\item The register-blocked GEMM micro-kernel makes Level-3 updates
  $10\times$ faster than hmatrix, amplifying the benefit of promoting
  work from Level-2 to Level-3.
\end{itemize}
...
%% ------------------------------------------------------------------
\subsubsection{4.\ Golub--Kahan SVD Benchmarking}
\label{sec:future-svdgkp-bench}

\paragraph{Current state.}
\texttt{svdGKP} (SVD.hs:322) is a complete Golub--Kahan bidiagonal
SVD implementation that avoids condition-number squaring, but is not
benchmarked or exposed as the default.  With the Round~17 GEMM
improvements cascading into the internal GEMM calls, \texttt{svdGKP}'s
relative cost profile may have shifted.

\paragraph{Proposed optimisation.}
Add \texttt{svdGKP} to the benchmark suite alongside \texttt{svdAtAP}
at $n = 100, 200, 500$.  If \texttt{svdGKP} is competitive after
the GEMM cascade, switch the default \texttt{svdP = svdGKP} to gain
both numerical robustness and performance.

\paragraph{Expected impact.}
$1.0\text{--}1.5\times$ potential improvement (unknown until
benchmarked), plus improved numerical stability for ill-conditioned
matrices.

%% ------------------------------------------------------------------
\subsubsection{5.\ GEMM $k$-Loop Unrolling}
\label{sec:future-gemm-unroll}

\paragraph{Current state.}
The $4{\times}8$ register-blocked micro-kernel (Kernel.hs:238)
processes one $k$ step per iteration: 4 scalar broadcasts from $A$,
2 vector loads from $B$, and 8 FMAs.  This achieves $10\times$ faster
than OpenBLAS at $500{\times}500$.

\paragraph{Proposed optimisation.}
Unroll the $k$-loop to $k_k \mathbin{{+}{=}} 2$ or $k_k \mathbin{{+}{=}} 4$:
prefetch $B[k{+}1, j..j{+}7]$ while computing the current FMAs.
This reduces branch prediction overhead and improves out-of-order
execution scheduling.  GHC~9.14's LLVM backend should handle the
register pressure from 16--32 simultaneous \texttt{DoubleX4\#} values.

\paragraph{Expected impact.}
$1.05\text{--}1.15\times$ GEMM improvement, cascading to eigenSH
and SVD via tridiagonalisation GEMM calls.

%% ------------------------------------------------------------------
\subsubsection{6.\ Cache Tile Size Tuning}
\label{sec:future-tile-tuning}

\paragraph{Current state.}
The GEMM tile size is $\mathtt{bs} = 64$ (Kernel.hs:156), giving a
$64 \times 64 \times 8 = 32$\,KB working set per tile---exactly
L1d size on AMD Zen.  The tridiagonalisation panel size is
$\mathtt{nb} = 48$ (Symmetric.hs:175).

\paragraph{Proposed optimisation.}
Benchmark tile sizes $\mathtt{bs} \in \{32, 48, 64, 96, 128\}$ and
panel sizes $\mathtt{nb} \in \{32, 48, 64\}$ across matrix sizes.
On CPUs with larger L1d (e.g., Apple M-series at 128\,KB) or larger
L2 (e.g., Intel Golden Cove at 1.25\,MB), larger tiles may improve
reuse.  Optionally, detect cache sizes at runtime via
\texttt{/sys/devices/system/cpu/cpu0/cache/} (Linux) or
\texttt{sysctl} (macOS).

\paragraph{Expected impact.}
$1.05\text{--}1.20\times$ on non-default hardware; no regression on
the development machine.

%% ------------------------------------------------------------------
\subsubsection{7.\ 2D Parallel GEMM Tiling}
...
\emph{entire} $B$ matrix.  On a 20-core system with
$500{\times}500$ matrices, all 20~threads contend for the same
$2$\,MB $B$ in L3 cache.
...
set by $\sqrt{p}$, improving L3 utilisation on multi-core systems.
Requires 2D thread indexing and per-block barrier synchronisation.

\paragraph{Expected impact.}
$1.2\text{--}1.4\times$ parallel GEMM improvement at 16{+} threads,
primarily by reducing L3 contention.  Most impactful on multi-socket
or chiplet architectures (AMD EPYC, Intel Xeon).

%% ------------------------------------------------------------------
\subsubsection{8.\ SIMD Substitution Unrolling}
\label{sec:future-subst-unroll}

\paragraph{Current state.}
Forward and backward substitution kernels
(\texttt{rawForwardSubUnitPackedSIMD}, \texttt{rawBackSubPackedSIMD}
in Kernel.hs) use 4-wide \texttt{DoubleX4\#} SIMD.  These kernels
execute during every LU and Cholesky solve.

\paragraph{Proposed optimisation.}
Unroll to 8-wide with two independent accumulators, following the
pattern applied to \texttt{rawMutSymMatvecSub} in Round~17.  Add a
4-wide cleanup loop and scalar tail for remainder elements.

\paragraph{Expected impact.}
$1.1\text{--}1.2\times$ improvement in LU/Cholesky solve, deepening
the $0.63\text{--}0.68\times$ advantage.

%% ------------------------------------------------------------------
\subsubsection{9.\ Persistent Column-Major Q for eigenSH}
\label{sec:future-colmajor-q}

\paragraph{Current state.}
\texttt{symmetricEigenPPar} (Symmetric.hs) transposes the eigenvector
matrix $Q$ to column-major before QR iteration and transposes back
afterwards---two $O(n^2)$ operations.  The column-major layout enables
SIMD Givens rotations on contiguous column elements.

\paragraph{Proposed optimisation.}
Maintain $Q$ in column-major layout throughout the entire eigenSH
pipeline, eliminating both transpose operations.  This requires
adjusting the tridiagonalisation Q-accumulation and all downstream
indexing to use column-major addressing.

\paragraph{Expected impact.}
$1.05\text{--}1.10\times$ improvement by saving $2 \times O(n^2)$
memory traffic, meaningful when QR iteration dominates (small $n$).

%% ------------------------------------------------------------------
\subsubsection{10.\ Panel Size Auto-Tuning}
\label{sec:future-panel-autotune}

\paragraph{Current state.}
Panel size $\mathtt{nb} = 48$ (Symmetric.hs) and GEMM tile size
$\mathtt{bs} = 64$ (Kernel.hs) are compile-time constants tuned on a
single machine (AMD Ryzen 9, 32\,KB L1d, 512\,KB L2, 32\,MB L3).

\paragraph{Proposed optimisation.}
Implement a one-time auto-tuning pass at library initialisation that
benchmarks $\mathtt{nb} \in \{32, 48, 64\}$ on a small test matrix
and caches the result.  Alternatively, select from a pre-validated
lookup table indexed by detected cache sizes.

\paragraph{Expected impact.}
$1.05\text{--}1.15\times$ on non-default hardware (especially Apple
M-series and Intel server CPUs).
...
1 & D\&C eigensolver crossover    & High   & Low    & $1.15\text{--}1.30\times$ & eigenSH ($n = 100\text{--}200$) \\
2 & Blocked bidiagonalisation     & High   & High   & $1.5\text{--}2.0\times$   & SVD ($n = 100\text{--}500$) \\
3 & Panel LU/Cholesky             & High   & Medium & $1.3\text{--}1.5\times$   & LU, Cholesky \\
4 & svdGKP benchmarking           & High   & Low    & $1.0\text{--}1.5\times$   & SVD \\
\midrule
\multicolumn{6}{l}{\textit{Medium priority --- deepening existing advantages}} \\
5 & GEMM $k$-loop unrolling       & Medium & Low    & $1.05\text{--}1.15\times$ & GEMM, eigenSH, SVD \\
6 & Cache tile size tuning         & Medium & Low    & $1.05\text{--}1.20\times$ & All operations \\
7 & 2D parallel GEMM tiling        & Medium & High   & $1.2\text{--}1.4\times$   & Parallel GEMM \\
8 & SIMD substitution unrolling    & Medium & Low    & $1.1\text{--}1.2\times$   & LU, Cholesky \\
\midrule
\multicolumn{6}{l}{\textit{Lower priority --- diminishing returns}} \\
9  & Persistent column-major Q     & Low    & Medium & $1.05\text{--}1.10\times$ & eigenSH (small $n$) \\
10 & Panel size auto-tuning        & Low    & Low    & $1.05\text{--}1.15\times$ & All (non-default HW) \\
\bottomrule
\end{tabular}
\end{center}

The four high-priority items target the two remaining areas where
\texttt{linear-massiv} is closest to hmatrix: eigenSH at medium sizes
($1.04\text{--}1.18\times$) and SVD at medium sizes ($1.80\times$).
D\&C crossover tuning and svdGKP benchmarking are quick wins (low
difficulty, high potential payoff).  Blocked bidiagonalisation is the
most complex item but addresses the widest remaining gap.  Panel-based
LU/Cholesky would deepen existing $0.63\text{--}0.68\times$ advantages
by promoting column-by-column Level-2 work to Level-3 GEMM, which
is now $10\times$ faster than OpenBLAS.
...
categories of numerical operations through seventeen rounds of optimisation.
...
nine}: GEMM ($0.10\text{--}0.16\times$, single-threaded and parallel),
dot product ($0.25\times$), matrix--vector multiply ($0.44\times$),
LU solve ($0.68\times$), Cholesky solve ($0.63\times$),
QR factorisation ($0.022\times$), eigenvalue decomposition
($0.51\text{--}1.18\times$), and SVD ($0.90\text{--}1.80\times$).
At $500{\times}500$---the largest benchmark size---every category
is faster than LAPACK, with eigenSH and SVD crossing the parity
