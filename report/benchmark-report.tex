...
After two rounds of optimisation---algorithmic improvements (cache-blocked
GEMM, in-place QR/eigenvalue via the ST monad) and raw
\texttt{ByteArray\#} with GHC~9.14's \texttt{DoubleX4\#} AVX2 SIMD
primops---\texttt{linear-massiv}'s BLAS Level~1--3 operations now
\textbf{outperform hmatrix (OpenBLAS)} at dimensions $\geq 50$:
GEMM is $2.1\times$ faster at $200 \times 200$, dot product is
$4\text{--}12\times$ faster, and matrix--vector multiply is
$1.6\text{--}2.2\times$ faster.  QR factorisation is within
$4\text{--}5\times$ of LAPACK.
\texttt{linear-massiv} demonstrates that pure Haskell with native SIMD
can match or exceed FFI-based BLAS, while providing compile-time
dimensional safety, zero FFI dependencies, and user-controllable
parallelism.
\end{abstract}
...
categories of numerical operations through three rounds of optimisation.
...
performance.  The result is transformative: \textbf{at matrix
dimensions $\geq 50$, \texttt{linear-massiv}'s pure Haskell GEMM
now outperforms \texttt{hmatrix} (OpenBLAS)}, achieving $2.1\times$
faster execution at $200 \times 200$.  The dot product is
$4\text{--}12\times$ faster than hmatrix at all sizes, and
matrix--vector multiply is $1.6\text{--}2.2\times$ faster at
$n \geq 50$.  Internal speedups range from $426\times$ (dot product)
to $793\times$ (matrix--vector multiply).

The remaining performance gaps are confined to higher-level algorithms
(LU, Cholesky, eigenvalue, SVD) that still use massiv's per-element
indexing in their inner loops.  Extending the raw kernel approach to
these algorithms---together with divide-and-conquer eigenvalue and SVD
algorithms---represents the clear path to achieving LAPACK-competitive
performance across the full operation suite.

\texttt{linear-massiv} now demonstrates that \textbf{pure Haskell with
GHC's native SIMD primops can match or exceed FFI-based BLAS
performance} for core linear algebra operations, while providing
compile-time dimensional safety, zero FFI dependencies, and
user-controllable parallelism.  This makes it a practical choice not
only for applications prioritising type safety and portability, but
increasingly for raw numerical throughput as well.
